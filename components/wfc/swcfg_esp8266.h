 ...
 * 
 * Code generated on 2021-08-31, 19:17:20 (CET).
 * 
...
class NodeConfig
{
	...
	*/
	...
	// optional string dns_server, id 6
	/*!
	* Function for querying if dns_server has been set.
	* @return true if dns_server is set.
	*/
	bool has_dns_server() const;
	//! Function to reset dns_server to its default/unset value.
	void clear_dns_server();
	//! Get value of dns_server.
	const estring &dns_server() const;
	/*!
	* Function for setting dns_server using binary data.
	* @param data pointer to binary data
	* @param s number of bytes at data pointer
	*/
	void set_dns_server(const void *data, size_t s);
	//! Set dns_server using a constant reference
	void set_dns_server(const estring &v);
	//! Set dns_server using a pointer to a null-terminated C-string.
	void set_dns_server(const char *);
	/*!
	* Provide mutable access to dns_server.
	* @return pointer to member variable of dns_server.
	*/
	estring *mutable_dns_server();
	...
	*/
	...
	//! Function get const-access to the elements of signals.
	const std::vector<SignalConfig> &signals() const;
	//! Function to get the number of elements in signals.
	size_t signals_size() const;
	/*!
	* Function to append a element to signals.
	* @return point to newly added element.
	*/
	SignalConfig* add_signals();
	//! Function to reset signals to its default/unset value.
	void clear_signals();
	//! Get value of element x of signals.
	const SignalConfig &signals(unsigned x) const;
	//! Set signals using a constant reference
	void set_signals(unsigned x, const SignalConfig &v);
	/*!
	* Provide mutable access to signals.
	* @return pointer to member variable of signals.
	*/
	SignalConfig *mutable_signals(unsigned x);
	//! Function to get mutable access to all elements of signals.
	std::vector<SignalConfig> *mutable_signals();
	#endif // CONFIG_SIGNAL_PROC
	
	#ifdef CONFIG_SIGNAL_PROC
	// repeated FunctionConfig functions, id 32
	//! Function get const-access to the elements of functions.
	const std::vector<FunctionConfig> &functions() const;
	//! Function to get the number of elements in functions.
	size_t functions_size() const;
	/*!
	* Function to append a element to functions.
	* @return point to newly added element.
	*/
	FunctionConfig* add_functions();
	//! Function to reset functions to its default/unset value.
	void clear_functions();
	//! Get value of element x of functions.
	const FunctionConfig &functions(unsigned x) const;
	//! Set functions using a constant reference
	void set_functions(unsigned x, const FunctionConfig &v);
	/*!
	* Provide mutable access to functions.
	* @return pointer to member variable of functions.
	*/
...
inline size_t NodeConfig::getMaxSize()
{
	...
	// optional string dns_server, id 6 has unlimited size
...
inline const estring &NodeConfig::dns_server() const
{
	return m_dns_server;
}
...
inline estring *NodeConfig::mutable_dns_server()
{
	return &m_dns_server;
}
...
inline void NodeConfig::set_dns_server(const estring &v)
{
	m_dns_server = v;
}
