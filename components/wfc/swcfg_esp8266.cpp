...
ssize_t EnvConfig::fromMemory(const void *b, ssize_t s)
{
	...
	while (a < e) {
		...
		case 0xa:	// path id 1, type estring, coding byte[]
			...
			m_path.assign((const char*)a,ud.vi);
			a += ud.vi;
			break;
		case 0x12:	// title id 2, type estring, coding byte[]
			...
			m_title.assign((const char*)a,ud.vi);
			a += ud.vi;
			break;
		case 0x1a:	// action id 3, type estring, coding byte[]
			if ((ssize_t)ud.vi > e-a) {
...
ssize_t ScreenConfig::fromMemory(const void *b, ssize_t s)
{
	...
	while (a < e) {
		...
		case 0xa:	// envs id 1, type EnvConfig, coding byte[]
			...
			if (((ssize_t)ud.vi > 0) && ((ssize_t)ud.vi <= (e-a))) {
				int n;
				n = m_envs.back().fromMemory((const uint8_t*)a,ud.vi);
				if (n != (ssize_t)ud.vi)
					return -248;
				a += ud.vi;
			}
		...
		case 0x22:	// font_tiny id 4, type estring, coding byte[]
			...
			m_font_tiny.assign((const char*)a,ud.vi);
			a += ud.vi;
			break;
		case 0x2a:	// font_small id 5, type estring, coding byte[]
			...
			m_font_small.assign((const char*)a,ud.vi);
			a += ud.vi;
			break;
		case 0x32:	// font_medium id 6, type estring, coding byte[]
			...
			m_font_medium.assign((const char*)a,ud.vi);
			a += ud.vi;
			break;
		case 0x3a:	// font_large id 7, type estring, coding byte[]
			...
			m_font_large.assign((const char*)a,ud.vi);
			a += ud.vi;
			break;
		default:
			if ((fid & 7) == 2) {
...
ssize_t ScreenConfig::toMemory(uint8_t *b, ssize_t s) const
{
	...
	if (!m_font_tiny.empty()) {
		...
		if (a >= e)
			return -257;
		*a++ = 0x22;
		n = encode_bytes(m_font_tiny,a,e);
		if (n < 0)
			return -258;
		a += n;
	}
	...
	if (!m_font_small.empty()) {
		...
		if (a >= e)
			return -259;
		*a++ = 0x2a;
		n = encode_bytes(m_font_small,a,e);
		if (n < 0)
			return -260;
		a += n;
	}
	...
	if (!m_font_medium.empty()) {
		...
		if (a >= e)
			return -261;
		*a++ = 0x32;
		n = encode_bytes(m_font_medium,a,e);
		if (n < 0)
			return -262;
		a += n;
	}
	...
	if (!m_font_large.empty()) {
		...
		if (a >= e)
			return -263;
		*a++ = 0x3a;
		n = encode_bytes(m_font_large,a,e);
		if (n < 0)
			return -264;
		a += n;
	}
...
ssize_t XPlaneConfig::fromMemory(const void *b, ssize_t s)
{
	...
	while (a < e) {
		...
		case 0x22:	// dataids id 4, type DataIdVar, coding byte[]
			...
			if (((ssize_t)ud.vi > 0) && ((ssize_t)ud.vi <= (e-a))) {
				int n;
				n = m_dataids.back().fromMemory((const uint8_t*)a,ud.vi);
				if (n != (ssize_t)ud.vi)
					return -293;
				a += ud.vi;
			}
		...
		case 0x2a:	// drefs id 5, type Dref, coding byte[]
			...
			if (((ssize_t)ud.vi > 0) && ((ssize_t)ud.vi <= (e-a))) {
				int n;
				n = m_drefs.back().fromMemory((const uint8_t*)a,ud.vi);
				if (n != (ssize_t)ud.vi)
					return -294;
				a += ud.vi;
			}
...
void XPlaneConfig::toJSON(stream &json, bool full, unsigned indLvl) const
{
	...
	if (full || has_refport()) {
		...
		to_decstr(json,m_refport);
	}
	if (full || has_dataport()) {
		fsep = json_indent(json,indLvl,fsep,"dataport");
		to_decstr(json,m_dataport);
	}
...
ssize_t NodeConfig::fromMemory(const void *b, ssize_t s)
{
	...
	while (a < e) {
		...
		case 0xda:	// screen id 27, type ScreenConfig, coding byte[]
			if (((ssize_t)ud.vi > 0) && ((ssize_t)ud.vi <= (e-a))) {
				int n;
				n = m_screen.fromMemory((const uint8_t*)a,ud.vi);
				if (n != (ssize_t)ud.vi)
					return -333;
				a += ud.vi;
			}
