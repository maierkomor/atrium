...
{
	static std::map<const char *,pull_mode_t, cstr_less> namesmap = {
		{ "pull_dir", pull_dir},
		{ "pull_down", pull_down},
		{ "pull_en", pull_en},
		{ "pull_none", pull_none},
...
{
	static std::map<const char *,relay_cfg_t, cstr_less> namesmap = {
		{ "rc_active_high", rc_active_high},
		{ "rc_active_low", rc_active_low},
		{ "rc_init_on", rc_init_on},
		{ "rc_mqtt", rc_mqtt},
		{ "rc_open_drain", rc_open_drain},
		{ "rc_persistent", rc_persistent},
	};
	char *e;
	unsigned long long ull = strtoull(s,&e,0);
	if (e != s) {
...
ssize_t GpioConfig::fromMemory(const void *b, ssize_t s)
{
	...
	while (a < e) {
		...
		case 0x10:	// gpio id 2, type int8_t, coding signed varint
			...
			break;
		case 0x18:	// config id 3, type uint64_t, coding varint
			{
				varint_t v;
				int n = read_varint(a,e-a,&v);
				if (n <= 0)
...
int GpioConfig::setByName(const char *name, const char *value)
{
	...
	if (0 == strcmp(name,"config")) {
		...
		int r = parse_ascii_u64(&m_config,value);
		if (r > 0)
			p_validbits |= ((uint8_t)1U << 1);
		return r;
	}
