...
ssize_t DisplayConfig::fromMemory(const void *b, ssize_t s)
{
	...
	while (a < e) {
		...
		case 0x20:	// maxy id 4, type uint64_t, coding varint
			...
			break;
		case 0x2a:	// skip_env id 5, type std::string, coding byte[]
			{
				varint_t v;
				int n = read_varint(a,e-a,&v);
				a += n;
				if ((n <= 0) || ((a+v) > e))
					return -347;
				m_skip_env.emplace_back((const char*)a,v);
				a += v;
			}
			break;
		default:
...
ssize_t DisplayConfig::toMemory(uint8_t *b, ssize_t s) const
{
	...
	for (const auto &x : m_skip_env) {
		...
		if (a >= e)
			return -358;
		*a++ = 0x2a;
		ssize_t skip_env_s = x.size();
		n = write_varint(a,e-a,skip_env_s);
		a += n;
		if ((n <= 0) || ((e-a) < skip_env_s))
			return -359;
		memcpy(a,x.data(),skip_env_s);
		a += skip_env_s;
	}
	assert(a <= e);
	return a-b;
...
void DisplayConfig::toWire(void (*put)(uint8_t)) const
{
	...
	for (size_t x = 0, y = m_skip_env.size(); x != y; ++x) {
		// 'skip_env': id=5, encoding=lenpfx, tag=0x2a
		put(0x2a);	// 'skip_env': id=5
		size_t skip_env_s = m_skip_env[x].size();
		send_varint(put,skip_env_s);
		send_bytes(put,(const uint8_t*) m_skip_env[x].data(),skip_env_s);
	}
}
...
void DisplayConfig::toString(std::string &put) const
{
	...
	for (size_t x = 0, y = m_skip_env.size(); x != y; ++x) {
		// 'skip_env': id=5, encoding=lenpfx, tag=0x2a
		put.push_back(0x2a);	// 'skip_env': id=5
		size_t skip_env_s = m_skip_env[x].size();
		send_varint(put,skip_env_s);
		put.append((const char *)(const uint8_t*) m_skip_env[x].data(),skip_env_s);
	}
}
...
void Tone::toASCII(std::ostream &o, size_t indent) const
{
	o << "Tone {";
	++indent;
	ascii_numeric(o, indent, "freq", m_freq);
...
ssize_t Tone::fromMemory(const void *b, ssize_t s)
{
	...
	while (a < e) {
		...
		a += fn;
		switch (fid) {
		case 0x8:	// freq id 1, type uint64_t, coding varint
			{
				varint_t v;
				int n = read_varint(a,e-a,&v);
				if (n <= 0)
			...
			break;
		case 0x10:	// duration id 2, type uint64_t, coding varint
			{
				varint_t v;
				int n = read_varint(a,e-a,&v);
				if (n <= 0)
...
void Tone::toWire(void (*put)(uint8_t)) const
{
	if (0 != (p_validbits & ((uint8_t)1U << 0))) {
		// 'freq': id=1, encoding=varint, tag=0x8
		put(0x8);	// 'freq': id=1
	...
	if (0 != (p_validbits & ((uint8_t)1U << 1))) {
		// 'duration': id=2, encoding=varint, tag=0x10
		put(0x10);	// 'duration': id=2
		send_varint(put,m_duration);
	}
}

void Tone::toString(std::string &put) const
{
	if (0 != (p_validbits & ((uint8_t)1U << 0))) {
		// 'freq': id=1, encoding=varint, tag=0x8
		put.push_back(0x8);	// 'freq': id=1
		send_varint(put,m_freq);
	}
	if (0 != (p_validbits & ((uint8_t)1U << 1))) {
		// 'duration': id=2, encoding=varint, tag=0x10
		put.push_back(0x10);	// 'duration': id=2
		send_varint(put,m_duration);
	}
}

void Tone::toJSON(std::ostream &json, unsigned indLvl) const
{
	char fsep = '{';
	++indLvl;
	if (has_freq()) {
...
int Tone::setByName(const char *name, const char *value)
{
	...
	if (0 == strcmp(name,"duration")) {
		...
		int r = parse_ascii_u64(&m_duration,value);
		if (r > 0)
			p_validbits |= ((uint8_t)1U << 1);
		return r;
	}
...
Message *Tone::p_getMember(const char *s, unsigned n)
{
	// caller guarantee: s[n] == '.'
	return 0;
}

Message *Tone::p_getMember(const char *s, unsigned n, unsigned x)
{
	// caller guarantee: s[n] == '['
	return 0;
}
...
void Melody::toASCII(std::ostream &o, size_t indent) const
{
	o << "Melody {";
	++indent;
	ascii_string(o,indent,m_name.data(),m_name.size(),"name");
...
void Melody::toJSON(std::ostream &json, unsigned indLvl) const
{
	char fsep = '{';
	++indLvl;
	if (has_name()) {
...
void BuzzerConfig::toASCII(std::ostream &o, size_t indent) const
{
	o << "BuzzerConfig {";
	++indent;
	ascii_numeric(o, indent, "gpio", (signed) m_gpio);
...
void BuzzerConfig::toJSON(std::ostream &json, unsigned indLvl) const
{
	char fsep = '{';
	++indLvl;
	if (has_gpio()) {
