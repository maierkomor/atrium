...
size_t parse_ascii_pull_mode_t(pull_mode_t *v, const char *s)
{
	char *e;
...
const char *relay_cfg_t_str(relay_cfg_t e)
{
	...
	default:
		return 0;
	case rc_active_low:
		return "rc_active_low";
	case rc_active_high:
		return "rc_active_high";
	case rc_init_on:
		return "rc_init_on";
	case rc_persistent:
		return "rc_persistent";
	case rc_open_drain:
		return "rc_open_drain";
	case rc_mqtt:
		return "rc_mqtt";
	}
}

size_t parse_ascii_relay_cfg_t(relay_cfg_t *v, const char *s)
{
	char *e;
	unsigned long long ull = strtoull(s,&e,0);
	if (e != s) {
		*v = (relay_cfg_t)ull;
		return e-s;
	} else if (0 == strcmp(s,"rc_active_low")) {
		*v = rc_active_low;
		return 13;
	} else if (0 == strcmp(s,"rc_active_high")) {
		*v = rc_active_high;
		return 14;
	} else if (0 == strcmp(s,"rc_init_on")) {
		*v = rc_init_on;
		return 10;
	} else if (0 == strcmp(s,"rc_persistent")) {
		*v = rc_persistent;
		return 13;
	} else if (0 == strcmp(s,"rc_open_drain")) {
		*v = rc_open_drain;
		return 13;
	} else if (0 == strcmp(s,"rc_mqtt")) {
		*v = rc_mqtt;
		return 7;
	} else {
		return 0;
...
void ButtonConfig::toASCII(stream &o, size_t indent) const
{
	...
	if (const char *v = pull_mode_t_str(m_pull_mode))
		o << v;
	else
		o << m_pull_mode;
	o << ';';
...
int ButtonConfig::setByName(const char *name, const char *value)
{
	...
	if (0 == strcmp(name,"pull_mode")) {
		if (value == 0) {
			clear_pull_mode();
			return 0;
		}
		pull_mode_t v;
		size_t r = parse_ascii_pull_mode_t(&v,value);
		if (r == 0)
			return -64;
		set_pull_mode(v);
		return r;
...
void RelayConfig::toASCII(stream &o, size_t indent) const
{
	...
	if (const char *v = relay_cfg_t_str(m_config))
		o << v;
	else
		o << m_config;
	o << ';';
...
int RelayConfig::setByName(const char *name, const char *value)
{
	...
	if (0 == strcmp(name,"config")) {
		...
		relay_cfg_t v;
		size_t r = parse_ascii_relay_cfg_t(&v,value);
		if (r == 0)
			return -80;
		set_config(v);
		return r;
...
void LedConfig::toJSON(stream &json, unsigned indLvl) const
{
	...
	if (has_config()) {
		fsep = json_indent(json,indLvl,fsep,"config");
		to_decstr(json,(unsigned) m_config);
	}
	if (has_name()) {
		fsep = json_indent(json,indLvl,fsep,"name");
		json_cstr(json,m_name.c_str());
...
size_t UartConfig::calcSize() const
{
	...
	// unused sint8 tx_gpio
	// unused sint8 rx_gpio
	// unused sint8 cts_gpio
	// unused sint8 rts_gpio
	return r;
}
...
size_t AdcConfig::calcSize() const
{
	...
	// unused uint8 adc1_bits
	// unused uint8 adc2_bits
	// uint8 mode
	if (has_mode()) {
	...
	// unused string hall_name
	// unused AdcChannel channels
	return r;
}
...
ssize_t HardwareConfig::fromMemory(const void *b, ssize_t s)
{
	...
	while (a < e) {
		...
		case 0x3a:	// gpio id 7, type GpioConfig, coding byte[]
			...
			if (((ssize_t)ud.vi > 0) && ((ssize_t)ud.vi <= (e-a))) {
				int n;
				n = m_gpio.back().fromMemory((const uint8_t*)a,ud.vi);
				if (n != (ssize_t)ud.vi)
					return -227;
				a += ud.vi;
			}
		...
		case 0x112:	// ws2812b id 34, type Ws2812bConfig, coding byte[]
			if (((ssize_t)ud.vi > 0) && ((ssize_t)ud.vi <= (e-a))) {
				int n;
				n = m_ws2812b.fromMemory((const uint8_t*)a,ud.vi);
				if (n != (ssize_t)ud.vi)
					return -233;
				a += ud.vi;
			}
...
size_t HardwareConfig::calcSize() const
{
	...
	// unused UartConfig uart
	// AdcConfig adc
	if (has_adc()) {
		size_t adc_s = m_adc.calcSize();
		r += adc_s + wiresize(adc_s) + 1 /* tag(adc) 0x20 */;
	}
	// unused TouchpadConfig touchpad
	// unused TouchChannelConfig tp_channel
	// GpioConfig gpio
