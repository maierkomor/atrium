...
inline uint64_t GpioConfig::config() const
{
	return m_config;
}
...
inline uint64_t *GpioConfig::mutable_config()
{
	if (0 == (p_validbits & ((uint8_t)1U << 1))) {
		p_validbits |= ((uint8_t)1U << 1);
		m_config = 0;
...
inline void GpioConfig::set_config(uint64_t  v)
{
	m_config = v;
	p_validbits |= ((uint8_t)1U << 1);
}
...
inline size_t DisplayConfig::getMaxSize()
{
	...
	// options has maximum size 11
	// maxx has maximum size 11
	// maxy has maximum size 11
	return 35;
}
