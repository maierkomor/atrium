 ...
 * WFC Version: P2304.22 (hg:478/f98d803ad48d)
 ...
 * 
 * Code generated on 2025-07-07, 23:21:37 (CET).
 * 
...
{
	return rstrsn_t_names[(size_t)e];
}
...
{
	return uart_sb_t_names[(size_t)e];
}
...
{
	return sigtype_t_names[(size_t)e];
}
...
void WifiConfig::toASCII(std::ostream &o, bool full, size_t indent) const
{
	o << "{ // WifiConfig";
	++indent;
	if (full || has_ssid()) {
...
void WifiConfig::toPbt(std::ostream &o, bool full, size_t indent) const
{
	o << '{';
	++indent;
	if (full || has_ssid()) {
		pbt_string(o,indent,m_ssid.data(),m_ssid.size(),"ssid");
	}
	if (full || has_pass()) {
		pbt_string(o,indent,m_pass.data(),m_pass.size(),"pass");
	}
	if (full || has_mac()) {
		ascii_bytes(o,(const uint8_t*)m_mac.data(),m_mac.size(),indent,"mac");
	}
	pbt_bool(o, indent, "activate", m_activate);
	if (full || has_addr4()) {
		pbt_numeric(o, indent, "addr4", m_addr4);
		o << "\t# ";
		ip4_to_ascii(o,m_addr4);
	}
	if (full || has_netmask4()) {
		pbt_numeric(o, indent, "netmask4", (unsigned) m_netmask4);
	}
	if (full || has_gateway4()) {
		pbt_numeric(o, indent, "gateway4", m_gateway4);
		o << "\t# ";
		ip4_to_ascii(o,m_gateway4);
	}
	--indent;
...
void WifiConfig::toJSON(std::ostream &json, bool full, unsigned indLvl) const
{
	char fsep = '{';
	++indLvl;
	if (full || has_ssid()) {
...
Message *WifiConfig::p_getMember(const char *s, unsigned n)
{
	// caller guarantee: s[n] == '.'
	return 0;
}

Message *WifiConfig::p_getMember(const char *s, unsigned n, unsigned x)
{
	// caller guarantee: s[n] == '['
	return 0;
}
...
void MQTT::toASCII(std::ostream &o, bool full, size_t indent) const
{
	o << "{ // MQTT";
	++indent;
	if (full || has_uri()) {
	...
	if (full || (0 != m_subscribtions.size())) {
		gen_indent(o,indent);
...
void MQTT::toPbt(std::ostream &o, bool full, size_t indent) const
{
	o << '{';
	++indent;
	if (full || has_uri()) {
		pbt_string(o,indent,m_uri.data(),m_uri.size(),"uri");
	}
	if (full || has_enable()) {
		pbt_bool(o, indent, "enable", m_enable);
	}
	if (full || has_username()) {
		pbt_string(o,indent,m_username.data(),m_username.size(),"username");
	}
	if (full || has_password()) {
		pbt_string(o,indent,m_password.data(),m_password.size(),"password");
	}
	if (full || (0 != m_subscribtions.size())) {
		gen_indent(o,indent);
		size_t s_subscribtions = m_subscribtions.size();
		...
		for (size_t i = 0, e = s_subscribtions; i != e; ++i) {
			gen_indent(o,indent);
			pbt_string(o,indent,m_subscribtions[i].data(),m_subscribtions[i].size(),0);
		}
		--indent;
		gen_indent(o,indent);
		o << ']';
	...
	if (full || has_keepalive()) {
		pbt_numeric(o, indent, "keepalive", m_keepalive);
	}
	--indent;
	gen_indent(o,indent);
	o << '}';
...
ssize_t MQTT::toMemory(uint8_t *b, ssize_t s) const
{
	...
	// has keepalive?
	if (0 != (p_validbits & ((uint8_t)1U << vb_keepalive))) {
...
void MQTT::toJSON(std::ostream &json, bool full, unsigned indLvl) const
{
	char fsep = '{';
	++indLvl;
	if (full || has_uri()) {
...
Message *MQTT::p_getMember(const char *s, unsigned n)
{
	// caller guarantee: s[n] == '.'
	return 0;
}

Message *MQTT::p_getMember(const char *s, unsigned n, unsigned x)
{
	// caller guarantee: s[n] == '['
	return 0;
}
...
void Date::toASCII(std::ostream &o, bool full, size_t indent) const
{
	o << "{ // Date";
	++indent;
	if (full || has_day()) {
...
void Date::toPbt(std::ostream &o, bool full, size_t indent) const
{
	o << '{';
	++indent;
	if (full || has_day()) {
		pbt_numeric(o, indent, "day", (unsigned) m_day);
	}
	if (full || has_month()) {
		pbt_numeric(o, indent, "month", (unsigned) m_month);
	}
	if (full || has_year()) {
		pbt_numeric(o, indent, "year", m_year);
	}
	if (full || has_endday()) {
		pbt_numeric(o, indent, "endday", (unsigned) m_endday);
	}
	if (full || has_endmonth()) {
		pbt_numeric(o, indent, "endmonth", (unsigned) m_endmonth);
	}
	if (full || has_endyear()) {
		pbt_numeric(o, indent, "endyear", (unsigned) m_endyear);
	}
	--indent;
	gen_indent(o,indent);
	o << '}';
...
void Date::toJSON(std::ostream &json, bool full, unsigned indLvl) const
{
	char fsep = '{';
	++indLvl;
	if (full || has_day()) {
...
Message *Date::p_getMember(const char *s, unsigned n)
{
	// caller guarantee: s[n] == '.'
	return 0;
}

Message *Date::p_getMember(const char *s, unsigned n, unsigned x)
{
	// caller guarantee: s[n] == '['
	return 0;
}
...
void AtAction::toASCII(std::ostream &o, bool full, size_t indent) const
{
	o << "{ // AtAction";
	++indent;
	if (full || has_day()) {
...
void AtAction::toPbt(std::ostream &o, bool full, size_t indent) const
{
	o << '{';
	++indent;
	if (full || has_day()) {
		pbt_indent(o,indent,"day");
		if (const char *v = WeekDay_str(m_day))
			o << v;
		else
			o << m_day;
	...
	if (full || has_min_of_day()) {
		pbt_numeric(o, indent, "min_of_day", m_min_of_day);
	}
	if (full || has_action()) {
		pbt_string(o,indent,m_action.data(),m_action.size(),"action");
	}
	pbt_bool(o, indent, "enable", m_enable);
	--indent;
	gen_indent(o,indent);
...
void AtAction::toJSON(std::ostream &json, bool full, unsigned indLvl) const
{
	char fsep = '{';
	++indLvl;
	if (full || has_day()) {
...
Message *AtAction::p_getMember(const char *s, unsigned n)
{
	// caller guarantee: s[n] == '.'
	return 0;
}

Message *AtAction::p_getMember(const char *s, unsigned n, unsigned x)
{
	// caller guarantee: s[n] == '['
	return 0;
}
...
void Influx::toASCII(std::ostream &o, bool full, size_t indent) const
{
	o << "{ // Influx";
	++indent;
...
void Influx::toPbt(std::ostream &o, bool full, size_t indent) const
{
	o << '{';
	++indent;
	if (full || has_hostname()) {
		pbt_string(o,indent,m_hostname.data(),m_hostname.size(),"hostname");
	}
	if (full || has_port()) {
		pbt_numeric(o, indent, "port", m_port);
	}
	if (full || has_measurement()) {
		pbt_string(o,indent,m_measurement.data(),m_measurement.size(),"measurement");
	}
	if (full || has_database()) {
		pbt_string(o,indent,m_database.data(),m_database.size(),"database");
	}
	--indent;
	gen_indent(o,indent);
	o << '}';
...
void Influx::toJSON(std::ostream &json, bool full, unsigned indLvl) const
{
	char fsep = '{';
	++indLvl;
	if (full || has_hostname()) {
...
Message *Influx::p_getMember(const char *s, unsigned n)
{
	// caller guarantee: s[n] == '.'
	return 0;
}

Message *Influx::p_getMember(const char *s, unsigned n, unsigned x)
{
	// caller guarantee: s[n] == '['
	return 0;
}
...
void UartSettings::toASCII(std::ostream &o, bool full, size_t indent) const
{
	o << "{ // UartSettings";
	++indent;
	if (full || has_port()) {
...
void UartSettings::toPbt(std::ostream &o, bool full, size_t indent) const
{
	o << '{';
	++indent;
	if (full || has_port()) {
	...
	if (full || has_baudrate()) {
		pbt_numeric(o, indent, "baudrate", m_baudrate);
	}
	if (full || has_config()) {
		pbt_numeric(o, indent, "config", m_config);
	}
	if (full || has_rx_thresh()) {
		pbt_numeric(o, indent, "rx_thresh", (unsigned) m_rx_thresh);
	}
	if (full || has_tx_bufsize()) {
		pbt_numeric(o, indent, "tx_bufsize", m_tx_bufsize);
	}
	if (full || has_rx_bufsize()) {
		pbt_numeric(o, indent, "rx_bufsize", m_rx_bufsize);
	}
	--indent;
	gen_indent(o,indent);
	o << '}';
...
void UartSettings::toJSON(std::ostream &json, bool full, unsigned indLvl) const
{
	char fsep = '{';
	++indLvl;
	if (full || has_port()) {
...
int UartSettings::setByName(const char *name, const char *value)
{
	...
	if ((0 == memcmp(name,"config",6)) && ((name[6] == 0) || name[6] == '.')) {
		...
		if (*name++ != '.') {
			...
			uart_wl_t tmp;
			int r = parse_ascii_uart_wl_t(&tmp,value);
			if (r > 0)
				set_config_wl(tmp);
			return r;
		} else if (!strcmp(name,"sb")) {
			uart_sb_t tmp;
			int r = parse_ascii_uart_sb_t(&tmp,value);
			if (r > 0)
				set_config_sb(tmp);
			return r;
		} else if (!strcmp(name,"rts")) {
			...
			uart_p_t tmp;
			int r = parse_ascii_uart_p_t(&tmp,value);
			if (r > 0)
				set_config_p(tmp);
			return r;
		} else if (!strcmp(name,"ref_tick")) {
...
Message *UartSettings::p_getMember(const char *s, unsigned n)
{
	// caller guarantee: s[n] == '.'
	return 0;
}

Message *UartSettings::p_getMember(const char *s, unsigned n, unsigned x)
{
	// caller guarantee: s[n] == '['
	return 0;
}
...
void FtpHttpConfig::toASCII(std::ostream &o, bool full, size_t indent) const
{
	o << "{ // FtpHttpConfig";
	++indent;
	if (full || has_port()) {
...
void FtpHttpConfig::toPbt(std::ostream &o, bool full, size_t indent) const
{
	o << '{';
	++indent;
	if (full || has_port()) {
	...
	if (full || has_start()) {
		pbt_bool(o, indent, "start", m_start);
	}
	if (full || has_root()) {
		pbt_string(o,indent,m_root.data(),m_root.size(),"root");
	}
	if (full || has_uploaddir()) {
		pbt_string(o,indent,m_uploaddir.data(),m_uploaddir.size(),"uploaddir");
	}
	--indent;
	gen_indent(o,indent);
	o << '}';
...
void FtpHttpConfig::toJSON(std::ostream &json, bool full, unsigned indLvl) const
{
	char fsep = '{';
	++indLvl;
	if (full || has_port()) {
...
Message *FtpHttpConfig::p_getMember(const char *s, unsigned n)
{
	// caller guarantee: s[n] == '.'
	return 0;
}

Message *FtpHttpConfig::p_getMember(const char *s, unsigned n, unsigned x)
{
	// caller guarantee: s[n] == '['
	return 0;
}
...
void TerminalConfig::toASCII(std::ostream &o, bool full, size_t indent) const
{
	o << "{ // TerminalConfig";
	++indent;
	if (full || has_uart_rx()) {
...
void TerminalConfig::toPbt(std::ostream &o, bool full, size_t indent) const
{
	o << '{';
	++indent;
	if (full || has_uart_rx()) {
		pbt_numeric(o, indent, "uart_rx", (signed) m_uart_rx);
	}
	if (full || has_uart_tx()) {
		pbt_numeric(o, indent, "uart_tx", (signed) m_uart_tx);
	}
	if (full || has_name()) {
...
void TerminalConfig::toJSON(std::ostream &json, bool full, unsigned indLvl) const
{
	char fsep = '{';
	++indLvl;
	if (full || has_uart_rx()) {
...
Message *TerminalConfig::p_getMember(const char *s, unsigned n)
{
	// caller guarantee: s[n] == '.'
	return 0;
}

Message *TerminalConfig::p_getMember(const char *s, unsigned n, unsigned x)
{
	// caller guarantee: s[n] == '['
	return 0;
}
...
void Trigger::toASCII(std::ostream &o, bool full, size_t indent) const
{
	o << "{ // Trigger";
	++indent;
	if (full || has_event()) {
...
void Trigger::toPbt(std::ostream &o, bool full, size_t indent) const
{
	o << '{';
	++indent;
	if (full || has_event()) {
		pbt_string(o,indent,m_event.data(),m_event.size(),"event");
	}
	if (full || (0 != m_action.size())) {
		gen_indent(o,indent);
		size_t s_action = m_action.size();
		...
		for (size_t i = 0, e = s_action; i != e; ++i) {
			gen_indent(o,indent);
			pbt_string(o,indent,m_action[i].data(),m_action[i].size(),0);
		}
		--indent;
		gen_indent(o,indent);
		o << ']';
...
void Trigger::toJSON(std::ostream &json, bool full, unsigned indLvl) const
{
	char fsep = '{';
	++indLvl;
	if (full || has_event()) {
...
Message *Trigger::p_getMember(const char *s, unsigned n)
{
	// caller guarantee: s[n] == '.'
	return 0;
}

Message *Trigger::p_getMember(const char *s, unsigned n, unsigned x)
{
	// caller guarantee: s[n] == '['
	return 0;
}
...
void AppParam::toASCII(std::ostream &o, bool full, size_t indent) const
{
	o << "{ // AppParam";
	++indent;
	if (full || has_key()) {
...
void AppParam::toPbt(std::ostream &o, bool full, size_t indent) const
{
	o << '{';
	++indent;
	if (full || has_key()) {
		pbt_string(o,indent,m_key.data(),m_key.size(),"key");
	}
	if (full || has_uValue()) {
		pbt_numeric(o, indent, "uValue", m_uValue);
	}
	if (full || has_sValue()) {
		pbt_string(o,indent,m_sValue.data(),m_sValue.size(),"sValue");
	}
	if (full || has_dValue()) {
		pbt_numeric(o, indent, "dValue", m_dValue);
	}
	if (full || has_fValue()) {
		pbt_numeric(o, indent, "fValue", m_fValue);
	}
	--indent;
	gen_indent(o,indent);
	o << '}';
...
ssize_t AppParam::toMemory(uint8_t *b, ssize_t s) const
{
	...
	// has uValue?
	if (0 != (p_validbits & ((uint8_t)1U << vb_uValue))) {
	...
	// has dValue?
	if (0 != (p_validbits & ((uint8_t)1U << vb_dValue))) {
	...
	// has fValue?
	if (0 != (p_validbits & ((uint8_t)1U << vb_fValue))) {
...
void AppParam::toJSON(std::ostream &json, bool full, unsigned indLvl) const
{
	char fsep = '{';
	++indLvl;
	if (full || has_key()) {
...
Message *AppParam::p_getMember(const char *s, unsigned n)
{
	// caller guarantee: s[n] == '.'
	return 0;
}

Message *AppParam::p_getMember(const char *s, unsigned n, unsigned x)
{
	// caller guarantee: s[n] == '['
	return 0;
}
...
void EventTimer::toASCII(std::ostream &o, bool full, size_t indent) const
{
	o << "{ // EventTimer";
	++indent;
	if (full || has_name()) {
	...
	if (full || has_config()) {
		ascii_indent(o,indent,"config");
		o << "{ // (eventcfg_t) " << (unsigned)m_config;
		++indent;
		gen_indent(o,indent);
		o << ".restart";
...
void EventTimer::toPbt(std::ostream &o, bool full, size_t indent) const
{
	o << '{';
	++indent;
	if (full || has_name()) {
	...
	if (full || has_time()) {
		pbt_numeric(o, indent, "time", m_time);
	}
	if (full || has_config()) {
...
void EventTimer::toJSON(std::ostream &json, bool full, unsigned indLvl) const
{
	char fsep = '{';
	++indLvl;
	if (full || has_name()) {
...
Message *EventTimer::p_getMember(const char *s, unsigned n)
{
	// caller guarantee: s[n] == '.'
	return 0;
}

Message *EventTimer::p_getMember(const char *s, unsigned n, unsigned x)
{
	// caller guarantee: s[n] == '['
	return 0;
}
...
void FunctionConfig::toASCII(std::ostream &o, bool full, size_t indent) const
{
	o << "{ // FunctionConfig";
	++indent;
	if (full || has_name()) {
...
void FunctionConfig::toPbt(std::ostream &o, bool full, size_t indent) const
{
	o << '{';
	++indent;
	if (full || has_name()) {
	...
	if (full || has_func()) {
		pbt_string(o,indent,m_func.data(),m_func.size(),"func");
	}
	if (full || (0 != m_params.size())) {
		gen_indent(o,indent);
		size_t s_params = m_params.size();
		...
		for (size_t i = 0, e = s_params; i != e; ++i) {
			gen_indent(o,indent);
			pbt_string(o,indent,m_params[i].data(),m_params[i].size(),0);
		}
		--indent;
		gen_indent(o,indent);
		o << ']';
...
void FunctionConfig::toJSON(std::ostream &json, bool full, unsigned indLvl) const
{
	char fsep = '{';
	++indLvl;
	if (full || has_name()) {
...
Message *FunctionConfig::p_getMember(const char *s, unsigned n)
{
	// caller guarantee: s[n] == '.'
	return 0;
}

Message *FunctionConfig::p_getMember(const char *s, unsigned n, unsigned x)
{
	// caller guarantee: s[n] == '['
	return 0;
}
...
void SignalConfig::toASCII(std::ostream &o, bool full, size_t indent) const
{
	o << "{ // SignalConfig";
	++indent;
	if (full || has_name()) {
...
void SignalConfig::toPbt(std::ostream &o, bool full, size_t indent) const
{
	o << '{';
	++indent;
	if (full || has_name()) {
	...
	if (full || has_iv()) {
		pbt_string(o,indent,m_iv.data(),m_iv.size(),"iv");
	}
	--indent;
	gen_indent(o,indent);
	o << '}';
...
void SignalConfig::toJSON(std::ostream &json, bool full, unsigned indLvl) const
{
	char fsep = '{';
	++indLvl;
	if (full || has_name()) {
...
Message *SignalConfig::p_getMember(const char *s, unsigned n)
{
	// caller guarantee: s[n] == '.'
	return 0;
}

Message *SignalConfig::p_getMember(const char *s, unsigned n, unsigned x)
{
	// caller guarantee: s[n] == '['
	return 0;
}
...
void OwDeviceConfig::toASCII(std::ostream &o, bool full, size_t indent) const
{
	o << "{ // OwDeviceConfig";
	++indent;
	if (full || has_id()) {
...
void OwDeviceConfig::toPbt(std::ostream &o, bool full, size_t indent) const
{
	o << '{';
	++indent;
	if (full || has_id()) {
		pbt_numeric(o, indent, "id", m_id);
		o << "\t# ";
		id64_to_ascii(o,m_id);
	}
	if (full || has_name()) {
...
void OwDeviceConfig::toJSON(std::ostream &json, bool full, unsigned indLvl) const
{
	char fsep = '{';
	++indLvl;
	if (full || has_id()) {
...
Message *OwDeviceConfig::p_getMember(const char *s, unsigned n)
{
	// caller guarantee: s[n] == '.'
	return 0;
}

Message *OwDeviceConfig::p_getMember(const char *s, unsigned n, unsigned x)
{
	// caller guarantee: s[n] == '['
	return 0;
}
...
void StateConfig::toASCII(std::ostream &o, bool full, size_t indent) const
{
	o << "{ // StateConfig";
	++indent;
	if (full || has_name()) {
	...
	if (full || (0 != m_conds.size())) {
		...
		for (size_t i = 0, e = s_conds; i != e; ++i) {
			gen_indent(o,indent);
			o << '[' << i << "]: ";
			m_conds[i].toASCII(o,full,indent);
		}
		--indent;
		gen_indent(o,indent);
		o << '}';
...
void StateConfig::toPbt(std::ostream &o, bool full, size_t indent) const
{
	o << '{';
	++indent;
	if (full || has_name()) {
...
void StateConfig::toJSON(std::ostream &json, bool full, unsigned indLvl) const
{
	char fsep = '{';
	++indLvl;
	if (full || has_name()) {
...
Message *StateConfig::p_getMember(const char *s, unsigned n)
{
	// caller guarantee: s[n] == '.'
	return 0;
}

Message *StateConfig::p_getMember(const char *s, unsigned n, unsigned x)
{
	// caller guarantee: s[n] == '['
	if ((n == 5) && (0 == memcmp("conds",s,5))) {
		if (x < m_conds.size())
			return &m_conds[x];
	}
	return 0;
...
void StateMachineConfig::toASCII(std::ostream &o, bool full, size_t indent) const
{
	...
	if (full || (0 != m_states.size())) {
		...
		for (size_t i = 0, e = s_states; i != e; ++i) {
			gen_indent(o,indent);
			o << '[' << i << "]: ";
			m_states[i].toASCII(o,full,indent);
		}
		--indent;
		gen_indent(o,indent);
		o << '}';
	}
	if (full || has_persistent()) {
...
void StateMachineConfig::toPbt(std::ostream &o, bool full, size_t indent) const
{
	o << '{';
	++indent;
	if (full || has_name()) {
	...
	if (full || has_ini_st()) {
		pbt_numeric(o, indent, "ini_st", (unsigned) m_ini_st);
	}
	if (full || (0 != m_states.size())) {
		gen_indent(o,indent);
		size_t s_states = m_states.size();
		...
		for (size_t i = 0, e = s_states; i != e; ++i) {
			gen_indent(o,indent);
			m_states[i].toPbt(o,full,indent);
		}
		--indent;
		gen_indent(o,indent);
		o << ']';
	...
	if (full || has_persistent()) {
		pbt_bool(o, indent, "persistent", m_persistent);
	}
	--indent;
	gen_indent(o,indent);
	o << '}';
...
ssize_t StateMachineConfig::toMemory(uint8_t *b, ssize_t s) const
{
	...
	// has ini_st?
	if (0 != (p_validbits & ((uint8_t)1U << vb_ini_st))) {
...
void StateMachineConfig::toString(std::string &put) const
{
	...
	if (m_persistent != false) {
		// 'persistent': id=4, encoding=8bit, tag=0x23
		put.push_back(0x23);	// 'persistent': id=4
		put.push_back(m_persistent);
	}
}

void StateMachineConfig::toJSON(std::ostream &json, bool full, unsigned indLvl) const
{
	char fsep = '{';
	++indLvl;
	if (full || has_name()) {
	...
	if (size_t s = m_states.size()) {
		...
		for (;;) {
			json_indent(json,indLvl,0);
			m_states[i].toJSON(json,full,indLvl);
			++i;
			if (i == s)
				break;
			json << ",\n";
...
Message *StateMachineConfig::p_getMember(const char *s, unsigned n)
{
	// caller guarantee: s[n] == '.'
	return 0;
}

Message *StateMachineConfig::p_getMember(const char *s, unsigned n, unsigned x)
{
	// caller guarantee: s[n] == '['
	if ((n == 6) && (0 == memcmp("states",s,6))) {
		if (x < m_states.size())
			return &m_states[x];
	}
	return 0;
...
void ThresholdConfig::toASCII(std::ostream &o, bool full, size_t indent) const
{
	o << "{ // ThresholdConfig";
	++indent;
	if (full || has_name()) {
...
void ThresholdConfig::toPbt(std::ostream &o, bool full, size_t indent) const
{
	o << '{';
	++indent;
	if (full || has_name()) {
	...
	if (full || has_low()) {
		pbt_numeric(o, indent, "low", m_low);
	}
	if (full || has_high()) {
		pbt_numeric(o, indent, "high", m_high);
	}
	--indent;
	gen_indent(o,indent);
	o << '}';
...
void ThresholdConfig::toJSON(std::ostream &json, bool full, unsigned indLvl) const
{
	char fsep = '{';
	++indLvl;
	if (full || has_name()) {
...
Message *ThresholdConfig::p_getMember(const char *s, unsigned n)
{
	// caller guarantee: s[n] == '.'
	return 0;
}

Message *ThresholdConfig::p_getMember(const char *s, unsigned n, unsigned x)
{
	// caller guarantee: s[n] == '['
	return 0;
}

void EnvConfig::clear()
{
	m_path.clear();
	m_title.clear();
	m_action.clear();
...
void EnvConfig::toASCII(std::ostream &o, bool full, size_t indent) const
{
	o << "{ // EnvConfig";
	++indent;
	if (full || has_path()) {
		ascii_string(o,indent,m_path.data(),m_path.size(),"path");
	}
	if (full || has_title()) {
		ascii_string(o,indent,m_title.data(),m_title.size(),"title");
	}
	if (full || has_action()) {
		ascii_string(o,indent,m_action.data(),m_action.size(),"action");
	}
...
void EnvConfig::toPbt(std::ostream &o, bool full, size_t indent) const
{
	o << '{';
	++indent;
	if (full || has_path()) {
		pbt_string(o,indent,m_path.data(),m_path.size(),"path");
	}
	if (full || has_title()) {
		pbt_string(o,indent,m_title.data(),m_title.size(),"title");
	}
	if (full || has_action()) {
		pbt_string(o,indent,m_action.data(),m_action.size(),"action");
	}
...
ssize_t EnvConfig::fromMemory(const void *b, ssize_t s)
{
	const uint8_t *a = (const uint8_t *)b;
	const uint8_t *e = a + s;
	while (a < e) {
		...
		a += fn;
		switch (fid) {
		case 0xa:	// path id 1, type std::string, coding byte[]
			{
				varint_t v;
				int n = read_varint(a,e-a,&v);
				a += n;
				if ((n <= 0) || ((a+v) > e))
					return -274;
				m_path.assign((const char*)a,v);
				a += v;
			}
			break;
		case 0x12:	// title id 2, type std::string, coding byte[]
			{
				varint_t v;
				int n = read_varint(a,e-a,&v);
				a += n;
...
ssize_t EnvConfig::toMemory(uint8_t *b, ssize_t s) const
{
	assert(s >= 0);
	uint8_t *a = b, *e = b + s;
	signed n;
	// has path?
	if (!m_path.empty()) {
		// 'path': id=1, encoding=lenpfx, tag=0xa
		if (a >= e)
			return -279;
		*a++ = 0xa;
		ssize_t path_s = m_path.size();
		n = write_varint(a,e-a,path_s);
		a += n;
		if ((n <= 0) || ((e-a) < path_s))
			return -280;
		memcpy(a,m_path.data(),path_s);
		a += path_s;
	}
	// has title?
	if (!m_title.empty()) {
		// 'title': id=2, encoding=lenpfx, tag=0x12
		if (a >= e)
			return -281;
		*a++ = 0x12;
		ssize_t title_s = m_title.size();
		n = write_varint(a,e-a,title_s);
		a += n;
		if ((n <= 0) || ((e-a) < title_s))
			return -282;
		memcpy(a,m_title.data(),title_s);
		a += title_s;
	}
	// has action?
	if (!m_action.empty()) {
...
void EnvConfig::toWire(void (*put)(uint8_t)) const
{
	if (!m_path.empty()) {
		// 'path': id=1, encoding=lenpfx, tag=0xa
		put(0xa);	// 'path': id=1
		size_t path_s = m_path.size();
		send_varint(put,path_s);
		send_bytes(put,(const uint8_t*) m_path.data(),path_s);
	}
	if (!m_title.empty()) {
		// 'title': id=2, encoding=lenpfx, tag=0x12
		put(0x12);	// 'title': id=2
		size_t title_s = m_title.size();
		send_varint(put,title_s);
		send_bytes(put,(const uint8_t*) m_title.data(),title_s);
	}
	if (!m_action.empty()) {
		// 'action': id=3, encoding=lenpfx, tag=0x1a
		put(0x1a);	// 'action': id=3
...
void EnvConfig::toString(std::string &put) const
{
	if (!m_path.empty()) {
		// 'path': id=1, encoding=lenpfx, tag=0xa
		put.push_back(0xa);	// 'path': id=1
		size_t path_s = m_path.size();
		send_varint(put,path_s);
		put.append((const char *)(const uint8_t*) m_path.data(),path_s);
	}
	if (!m_title.empty()) {
		// 'title': id=2, encoding=lenpfx, tag=0x12
		put.push_back(0x12);	// 'title': id=2
		size_t title_s = m_title.size();
		send_varint(put,title_s);
		put.append((const char *)(const uint8_t*) m_title.data(),title_s);
	}
	if (!m_action.empty()) {
		// 'action': id=3, encoding=lenpfx, tag=0x1a
		put.push_back(0x1a);	// 'action': id=3
...
void EnvConfig::toJSON(std::ostream &json, bool full, unsigned indLvl) const
{
	char fsep = '{';
	++indLvl;
	if (full || has_path()) {
		fsep = json_indent(json,indLvl,fsep,"path");
		json_cstr(json,m_path.c_str());
	}
	if (full || has_title()) {
		fsep = json_indent(json,indLvl,fsep,"title");
		json_cstr(json,m_title.c_str());
	}
	if (full || has_action()) {
		fsep = json_indent(json,indLvl,fsep,"action");
		json_cstr(json,m_action.c_str());
...
size_t EnvConfig::calcSize() const
{
	size_t r = 0;	// required size, default is fixed length
	// optional string path, id 1
	if (!m_path.empty()) {
		size_t path_s = m_path.size();
		r += path_s + wiresize(path_s) + 1 /* tag(path) 0x8 */;
	}
	// optional string title, id 2
	if (!m_title.empty()) {
		size_t title_s = m_title.size();
		r += title_s + wiresize(title_s) + 1 /* tag(title) 0x10 */;
	}
	// optional string action, id 3
	if (!m_action.empty()) {
		size_t action_s = m_action.size();
...
bool EnvConfig::operator != (const EnvConfig &r) const
{
	if (has_path() && (m_path != r.m_path))
		return true;
	if (has_title() && (m_title != r.m_title))
		return true;
	if (has_action() && (m_action != r.m_action))
		return true;
	return false;
 ...
 *         an error.
 */
int EnvConfig::setByName(const char *name, const char *value)
{
...
Message *EnvConfig::p_getMember(const char *s, unsigned n)
{
	// caller guarantee: s[n] == '.'
	return 0;
}

Message *EnvConfig::p_getMember(const char *s, unsigned n, unsigned x)
{
	// caller guarantee: s[n] == '['
	return 0;
}
...
void ScreenConfig::toASCII(std::ostream &o, bool full, size_t indent) const
{
	o << "{ // ScreenConfig";
	++indent;
	if (full || (0 != m_envs.size())) {
		gen_indent(o,indent);
		size_t s_envs = m_envs.size();
		o << "envs[" << s_envs << "] = {";
		++indent;
		for (size_t i = 0, e = s_envs; i != e; ++i) {
			gen_indent(o,indent);
			o << '[' << i << "]: ";
			m_envs[i].toASCII(o,full,indent);
		}
		--indent;
		gen_indent(o,indent);
		o << '}';
	}
	if (full || has_font_tiny()) {
		ascii_string(o,indent,m_font_tiny.data(),m_font_tiny.size(),"font_tiny");
	}
	if (full || has_font_small()) {
		ascii_string(o,indent,m_font_small.data(),m_font_small.size(),"font_small");
	}
	if (full || has_font_medium()) {
		ascii_string(o,indent,m_font_medium.data(),m_font_medium.size(),"font_medium");
	}
	if (full || has_font_large()) {
		ascii_string(o,indent,m_font_large.data(),m_font_large.size(),"font_large");
	}
	--indent;
	gen_indent(o,indent);
	o << '}';
...
void ScreenConfig::toPbt(std::ostream &o, bool full, size_t indent) const
{
	o << '{';
	++indent;
	if (full || (0 != m_envs.size())) {
	...
	if (full || has_font_tiny()) {
		pbt_string(o,indent,m_font_tiny.data(),m_font_tiny.size(),"font_tiny");
	}
	if (full || has_font_small()) {
		pbt_string(o,indent,m_font_small.data(),m_font_small.size(),"font_small");
	}
	if (full || has_font_medium()) {
		pbt_string(o,indent,m_font_medium.data(),m_font_medium.size(),"font_medium");
	}
	if (full || has_font_large()) {
		pbt_string(o,indent,m_font_large.data(),m_font_large.size(),"font_large");
	}
	--indent;
	gen_indent(o,indent);
	o << '}';
...
ssize_t ScreenConfig::fromMemory(const void *b, ssize_t s)
{
	const uint8_t *a = (const uint8_t *)b;
	const uint8_t *e = a + s;
	while (a < e) {
		...
		a += fn;
		switch (fid) {
		case 0xa:	// envs id 1, type EnvConfig, coding byte[]
			{
				varint_t v;
				int n = read_varint(a,e-a,&v);
				a += n;
				...
				m_envs.emplace_back();
				if (v != 0) {
					n = m_envs.back().fromMemory((const uint8_t*)a,v);
					if (n < 0)
						return n;
					if (n != (ssize_t)v)
						return -288;
			...
			break;
		case 0x22:	// font_tiny id 4, type std::string, coding byte[]
			{
				varint_t v;
				int n = read_varint(a,e-a,&v);
				a += n;
				if ((n <= 0) || ((a+v) > e))
					return -289;
				m_font_tiny.assign((const char*)a,v);
				a += v;
			}
			break;
		case 0x2a:	// font_small id 5, type std::string, coding byte[]
			{
				varint_t v;
				int n = read_varint(a,e-a,&v);
				a += n;
				if ((n <= 0) || ((a+v) > e))
					return -290;
				m_font_small.assign((const char*)a,v);
				a += v;
			}
			break;
		case 0x32:	// font_medium id 6, type std::string, coding byte[]
			{
				varint_t v;
				int n = read_varint(a,e-a,&v);
				a += n;
				if ((n <= 0) || ((a+v) > e))
					return -291;
				m_font_medium.assign((const char*)a,v);
				a += v;
			}
			break;
		case 0x3a:	// font_large id 7, type std::string, coding byte[]
			{
				varint_t v;
				int n = read_varint(a,e-a,&v);
				a += n;
				if ((n <= 0) || ((a+v) > e))
					return -292;
				m_font_large.assign((const char*)a,v);
				a += v;
			}
			break;
		default:
...
ssize_t ScreenConfig::toMemory(uint8_t *b, ssize_t s) const
{
	assert(s >= 0);
	uint8_t *a = b, *e = b + s;
	signed n;
	for (const auto &x : m_envs) {
		// 'envs': id=1, encoding=lenpfx, tag=0xa
		if (a >= e)
			return -295;
		*a++ = 0xa;
		ssize_t envs_ws = x.calcSize();
		n = write_varint(a,e-a,envs_ws);
		a += n;
		if ((n <= 0) || (envs_ws > (e-a)))
			return -296;
		n = x.toMemory(a,e-a);
	...
	// has font_tiny?
	if (!m_font_tiny.empty()) {
		// 'font_tiny': id=4, encoding=lenpfx, tag=0x22
		if (a >= e)
			return -297;
		*a++ = 0x22;
		ssize_t font_tiny_s = m_font_tiny.size();
		n = write_varint(a,e-a,font_tiny_s);
		a += n;
		if ((n <= 0) || ((e-a) < font_tiny_s))
			return -298;
		memcpy(a,m_font_tiny.data(),font_tiny_s);
		a += font_tiny_s;
	}
	// has font_small?
	if (!m_font_small.empty()) {
		// 'font_small': id=5, encoding=lenpfx, tag=0x2a
		if (a >= e)
			return -299;
		*a++ = 0x2a;
		ssize_t font_small_s = m_font_small.size();
		n = write_varint(a,e-a,font_small_s);
		a += n;
		if ((n <= 0) || ((e-a) < font_small_s))
			return -300;
		memcpy(a,m_font_small.data(),font_small_s);
		a += font_small_s;
	}
	// has font_medium?
	if (!m_font_medium.empty()) {
		// 'font_medium': id=6, encoding=lenpfx, tag=0x32
		if (a >= e)
			return -301;
		*a++ = 0x32;
		ssize_t font_medium_s = m_font_medium.size();
		n = write_varint(a,e-a,font_medium_s);
		a += n;
		if ((n <= 0) || ((e-a) < font_medium_s))
			return -302;
		memcpy(a,m_font_medium.data(),font_medium_s);
		a += font_medium_s;
	}
	// has font_large?
	if (!m_font_large.empty()) {
		// 'font_large': id=7, encoding=lenpfx, tag=0x3a
		if (a >= e)
			return -303;
		*a++ = 0x3a;
		ssize_t font_large_s = m_font_large.size();
		n = write_varint(a,e-a,font_large_s);
		a += n;
		if ((n <= 0) || ((e-a) < font_large_s))
			return -304;
		memcpy(a,m_font_large.data(),font_large_s);
		a += font_large_s;
	}
	assert(a <= e);
	return a-b;
...
void ScreenConfig::toWire(void (*put)(uint8_t)) const
{
	for (size_t x = 0, y = m_envs.size(); x != y; ++x) {
		// 'envs': id=1, encoding=lenpfx, tag=0xa
		put(0xa);	// 'envs': id=1
		send_varint(put,m_envs[x].calcSize());
		m_envs[x].toWire(put);
	}
	if (!m_font_tiny.empty()) {
		// 'font_tiny': id=4, encoding=lenpfx, tag=0x22
		put(0x22);	// 'font_tiny': id=4
		size_t font_tiny_s = m_font_tiny.size();
		send_varint(put,font_tiny_s);
		send_bytes(put,(const uint8_t*) m_font_tiny.data(),font_tiny_s);
	}
	if (!m_font_small.empty()) {
		// 'font_small': id=5, encoding=lenpfx, tag=0x2a
		put(0x2a);	// 'font_small': id=5
		size_t font_small_s = m_font_small.size();
		send_varint(put,font_small_s);
		send_bytes(put,(const uint8_t*) m_font_small.data(),font_small_s);
	}
	if (!m_font_medium.empty()) {
		// 'font_medium': id=6, encoding=lenpfx, tag=0x32
		put(0x32);	// 'font_medium': id=6
		size_t font_medium_s = m_font_medium.size();
		send_varint(put,font_medium_s);
		send_bytes(put,(const uint8_t*) m_font_medium.data(),font_medium_s);
	}
	if (!m_font_large.empty()) {
		// 'font_large': id=7, encoding=lenpfx, tag=0x3a
		put(0x3a);	// 'font_large': id=7
		size_t font_large_s = m_font_large.size();
		send_varint(put,font_large_s);
		send_bytes(put,(const uint8_t*) m_font_large.data(),font_large_s);
	}
}

void ScreenConfig::toString(std::string &put) const
{
	for (size_t x = 0, y = m_envs.size(); x != y; ++x) {
		// 'envs': id=1, encoding=lenpfx, tag=0xa
		put.push_back(0xa);	// 'envs': id=1
		send_varint(put,m_envs[x].calcSize());
		m_envs[x].toString(put);
	}
	if (!m_font_tiny.empty()) {
		// 'font_tiny': id=4, encoding=lenpfx, tag=0x22
		put.push_back(0x22);	// 'font_tiny': id=4
		size_t font_tiny_s = m_font_tiny.size();
		send_varint(put,font_tiny_s);
		put.append((const char *)(const uint8_t*) m_font_tiny.data(),font_tiny_s);
	}
	if (!m_font_small.empty()) {
		// 'font_small': id=5, encoding=lenpfx, tag=0x2a
		put.push_back(0x2a);	// 'font_small': id=5
		size_t font_small_s = m_font_small.size();
		send_varint(put,font_small_s);
		put.append((const char *)(const uint8_t*) m_font_small.data(),font_small_s);
	}
	if (!m_font_medium.empty()) {
		// 'font_medium': id=6, encoding=lenpfx, tag=0x32
		put.push_back(0x32);	// 'font_medium': id=6
		size_t font_medium_s = m_font_medium.size();
		send_varint(put,font_medium_s);
		put.append((const char *)(const uint8_t*) m_font_medium.data(),font_medium_s);
	}
	if (!m_font_large.empty()) {
		// 'font_large': id=7, encoding=lenpfx, tag=0x3a
		put.push_back(0x3a);	// 'font_large': id=7
		size_t font_large_s = m_font_large.size();
		send_varint(put,font_large_s);
		put.append((const char *)(const uint8_t*) m_font_large.data(),font_large_s);
	}
}

void ScreenConfig::toJSON(std::ostream &json, bool full, unsigned indLvl) const
{
	char fsep = '{';
	++indLvl;
	if (size_t s = m_envs.size()) {
		fsep = json_indent(json,indLvl,fsep);
		indLvl += 2;
		json << "\"envs\":[\n";
		size_t i = 0;
		for (;;) {
			json_indent(json,indLvl,0);
			m_envs[i].toJSON(json,full,indLvl);
			++i;
			if (i == s)
				break;
			json << ",\n";
	...
	if (full || has_font_tiny()) {
		fsep = json_indent(json,indLvl,fsep,"font_tiny");
		json_cstr(json,m_font_tiny.c_str());
	}
	if (full || has_font_small()) {
		fsep = json_indent(json,indLvl,fsep,"font_small");
		json_cstr(json,m_font_small.c_str());
	}
	if (full || has_font_medium()) {
		fsep = json_indent(json,indLvl,fsep,"font_medium");
		json_cstr(json,m_font_medium.c_str());
	}
	if (full || has_font_large()) {
		fsep = json_indent(json,indLvl,fsep,"font_large");
		json_cstr(json,m_font_large.c_str());
	}
	if (fsep == '{')
		json.put('{');
	json.put('\n');
...
size_t ScreenConfig::calcSize() const
{
	size_t r = 0;	// required size, default is fixed length
	// repeated EnvConfig envs, id 1
	// repeated message envs
	for (size_t x = 0, y = m_envs.size(); x < y; ++x) {
		size_t s = m_envs[x].calcSize();
		r += wiresize(s);
		r += s + 1 /* tag(envs) 0x8 */;
	}
	...
	if (!m_font_tiny.empty()) {
		size_t font_tiny_s = m_font_tiny.size();
		r += font_tiny_s + wiresize(font_tiny_s) + 1 /* tag(font_tiny) 0x20 */;
	}
	// optional string font_small, id 5
	if (!m_font_small.empty()) {
		size_t font_small_s = m_font_small.size();
		r += font_small_s + wiresize(font_small_s) + 1 /* tag(font_small) 0x28 */;
	}
	// optional string font_medium, id 6
	if (!m_font_medium.empty()) {
		size_t font_medium_s = m_font_medium.size();
		r += font_medium_s + wiresize(font_medium_s) + 1 /* tag(font_medium) 0x30 */;
	}
	// optional string font_large, id 7
	if (!m_font_large.empty()) {
		size_t font_large_s = m_font_large.size();
		r += font_large_s + wiresize(font_large_s) + 1 /* tag(font_large) 0x38 */;
	}
	return r;
}

bool ScreenConfig::operator != (const ScreenConfig &r) const
{
	if (m_envs != r.m_envs)
		return true;
	if (has_font_tiny() && (m_font_tiny != r.m_font_tiny))
		return true;
	if (has_font_small() && (m_font_small != r.m_font_small))
		return true;
	if (has_font_medium() && (m_font_medium != r.m_font_medium))
		return true;
	if (has_font_large() && (m_font_large != r.m_font_large))
		return true;
	return false;
}
 ...
 *         an error.
 */
int ScreenConfig::setByName(const char *name, const char *value)
{
	if (0 == memcmp(name,"envs",4)) {
		if ((name[4] == 0) && (value == 0)) {
			clear_envs();
			return 0;
		} else if (name[4] == '[') {
			...
			if ((name[5] == '+') && (name[6] == ']')) {
				...
				if (m_envs.size() <= x)
					return -306;
				if ((idxe[1] == 0) && (value == 0)) {
					m_envs.erase(m_envs.begin()+x);
					return 0;
				}
			}
			if (idxe[1] != '.')
				return -307;
			return m_envs[x].setByName(idxe+2,value);
		}
	}
	if (0 == strcmp(name,"font_tiny")) {
		if (value == 0) {
			m_font_tiny.clear();
			return 0;
		}
		m_font_tiny = value;
		int r = m_font_tiny.size();
		return r;
	}
	if (0 == strcmp(name,"font_small")) {
		if (value == 0) {
			m_font_small.clear();
			return 0;
		}
		m_font_small = value;
		int r = m_font_small.size();
		return r;
	}
	if (0 == strcmp(name,"font_medium")) {
		if (value == 0) {
			m_font_medium.clear();
			return 0;
		}
		m_font_medium = value;
		int r = m_font_medium.size();
		return r;
	}
	if (0 == strcmp(name,"font_large")) {
		if (value == 0) {
			m_font_large.clear();
			return 0;
		}
		m_font_large = value;
		int r = m_font_large.size();
		return r;
	}
	return -308;
}

Message *ScreenConfig::p_getMember(const char *s, unsigned n)
{
	// caller guarantee: s[n] == '.'
	return 0;
}

Message *ScreenConfig::p_getMember(const char *s, unsigned n, unsigned x)
{
	// caller guarantee: s[n] == '['
	if ((n == 4) && (0 == memcmp("envs",s,4))) {
		if (x < m_envs.size())
			return &m_envs[x];
	}
	return 0;
...
void Dref::toASCII(std::ostream &o, bool full, size_t indent) const
{
	o << "{ // Dref";
	++indent;
	if (full || has_name()) {
	...
	if (full || has_alias()) {
		ascii_string(o,indent,m_alias.data(),m_alias.size(),"alias");
	}
	if (full || has_freq()) {
...
void Dref::toPbt(std::ostream &o, bool full, size_t indent) const
{
	o << '{';
	++indent;
	if (full || has_name()) {
	...
	if (full || has_alias()) {
		pbt_string(o,indent,m_alias.data(),m_alias.size(),"alias");
	}
	if (full || has_freq()) {
...
ssize_t Dref::fromMemory(const void *b, ssize_t s)
{
	...
	while (a < e) {
		...
		case 0xa:	// name id 1, type std::string, coding byte[]
			...
			break;
		case 0x12:	// alias id 2, type std::string, coding byte[]
			{
				varint_t v;
				int n = read_varint(a,e-a,&v);
				a += n;
				if ((n <= 0) || ((a+v) > e))
					return -311;
				m_alias.assign((const char*)a,v);
				a += v;
			}
			break;
		case 0x18:	// freq id 3, type uint64_t, coding varint
			{
				varint_t v;
				int n = read_varint(a,e-a,&v);
				if (n <= 0)
...
ssize_t Dref::toMemory(uint8_t *b, ssize_t s) const
{
	...
	if (!m_alias.empty()) {
		...
		if (a >= e)
			return -317;
		*a++ = 0x12;
		ssize_t alias_s = m_alias.size();
		n = write_varint(a,e-a,alias_s);
		a += n;
		if ((n <= 0) || ((e-a) < alias_s))
			return -318;
		memcpy(a,m_alias.data(),alias_s);
		a += alias_s;
	}
	// has freq?
...
void Dref::toWire(void (*put)(uint8_t)) const
{
	if (!m_name.empty()) {
		// 'name': id=1, encoding=lenpfx, tag=0xa
		put(0xa);	// 'name': id=1
	...
	if (!m_alias.empty()) {
		...
		send_varint(put,alias_s);
		send_bytes(put,(const uint8_t*) m_alias.data(),alias_s);
	}
	if (0 != (p_validbits & ((uint8_t)1U << vb_freq))) {
		// 'freq': id=3, encoding=varint, tag=0x18
		put(0x18);	// 'freq': id=3
...
void Dref::toString(std::string &put) const
{
	if (!m_name.empty()) {
		// 'name': id=1, encoding=lenpfx, tag=0xa
		put.push_back(0xa);	// 'name': id=1
	...
	if (!m_alias.empty()) {
		// 'alias': id=2, encoding=lenpfx, tag=0x12
		put.push_back(0x12);	// 'alias': id=2
		size_t alias_s = m_alias.size();
		send_varint(put,alias_s);
		put.append((const char *)(const uint8_t*) m_alias.data(),alias_s);
	}
	if (0 != (p_validbits & ((uint8_t)1U << vb_freq))) {
		// 'freq': id=3, encoding=varint, tag=0x18
		put.push_back(0x18);	// 'freq': id=3
		send_varint(put,m_freq);
	}
}

void Dref::toJSON(std::ostream &json, bool full, unsigned indLvl) const
{
	char fsep = '{';
	++indLvl;
	if (full || has_name()) {
...
Message *Dref::p_getMember(const char *s, unsigned n)
{
	// caller guarantee: s[n] == '.'
	return 0;
}

Message *Dref::p_getMember(const char *s, unsigned n, unsigned x)
{
	// caller guarantee: s[n] == '['
	return 0;
}
...
void DataIdVar::toASCII(std::ostream &o, bool full, size_t indent) const
{
	o << "{ // DataIdVar";
	++indent;
	if (full || has_id()) {
		ascii_numeric(o, indent, "id", m_id);
	}
	if (full || has_idx()) {
		ascii_numeric(o, indent, "idx", m_idx);
	}
	if (full || has_varname()) {
		ascii_string(o,indent,m_varname.data(),m_varname.size(),"varname");
	}
	--indent;
	gen_indent(o,indent);
	o << '}';
...
void DataIdVar::toPbt(std::ostream &o, bool full, size_t indent) const
{
	o << '{';
	++indent;
	if (full || has_id()) {
	...
	if (full || has_idx()) {
		pbt_numeric(o, indent, "idx", m_idx);
	}
	if (full || has_varname()) {
		pbt_string(o,indent,m_varname.data(),m_varname.size(),"varname");
	}
	--indent;
	gen_indent(o,indent);
	o << '}';
...
ssize_t DataIdVar::fromMemory(const void *b, ssize_t s)
{
	...
	while (a < e) {
		...
		a += fn;
		switch (fid) {
		case 0x8:	// id id 1, type uint64_t, coding varint
			{
				varint_t v;
				int n = read_varint(a,e-a,&v);
				if (n <= 0)
			...
			break;
		case 0x10:	// idx id 2, type uint64_t, coding varint
			{
				varint_t v;
				int n = read_varint(a,e-a,&v);
				if (n <= 0)
			...
			break;
		case 0x1a:	// varname id 3, type std::string, coding byte[]
			{
				varint_t v;
				int n = read_varint(a,e-a,&v);
				a += n;
				if ((n <= 0) || ((a+v) > e))
					return -325;
				m_varname.assign((const char*)a,v);
				a += v;
			}
			break;
		default:
...
ssize_t DataIdVar::toMemory(uint8_t *b, ssize_t s) const
{
	...
	if (!m_varname.empty()) {
		...
		if (a >= e)
			return -332;
		*a++ = 0x1a;
		ssize_t varname_s = m_varname.size();
		n = write_varint(a,e-a,varname_s);
		a += n;
		if ((n <= 0) || ((e-a) < varname_s))
			return -333;
		memcpy(a,m_varname.data(),varname_s);
		a += varname_s;
	}
	assert(a <= e);
	return a-b;
...
void DataIdVar::toWire(void (*put)(uint8_t)) const
{
	if (0 != (p_validbits & ((uint8_t)1U << vb_id))) {
		// 'id': id=1, encoding=varint, tag=0x8
		put(0x8);	// 'id': id=1
		send_varint(put,m_id);
	}
	if (m_idx != 0) {
		// 'idx': id=2, encoding=varint, tag=0x10
		put(0x10);	// 'idx': id=2
		send_varint(put,m_idx);
	}
	if (!m_varname.empty()) {
		// 'varname': id=3, encoding=lenpfx, tag=0x1a
		put(0x1a);	// 'varname': id=3
		size_t varname_s = m_varname.size();
		send_varint(put,varname_s);
		send_bytes(put,(const uint8_t*) m_varname.data(),varname_s);
	}
}

void DataIdVar::toString(std::string &put) const
{
	if (0 != (p_validbits & ((uint8_t)1U << vb_id))) {
		// 'id': id=1, encoding=varint, tag=0x8
		put.push_back(0x8);	// 'id': id=1
		send_varint(put,m_id);
	}
	if (m_idx != 0) {
		// 'idx': id=2, encoding=varint, tag=0x10
		put.push_back(0x10);	// 'idx': id=2
		send_varint(put,m_idx);
	}
	if (!m_varname.empty()) {
		// 'varname': id=3, encoding=lenpfx, tag=0x1a
		put.push_back(0x1a);	// 'varname': id=3
		size_t varname_s = m_varname.size();
		send_varint(put,varname_s);
		put.append((const char *)(const uint8_t*) m_varname.data(),varname_s);
	}
}

void DataIdVar::toJSON(std::ostream &json, bool full, unsigned indLvl) const
{
	char fsep = '{';
	++indLvl;
	if (full || has_id()) {
...
int DataIdVar::setByName(const char *name, const char *value)
{
	...
	if (0 == strcmp(name,"idx")) {
		...
		int r = parse_ascii_u64(&m_idx,value);
		return r;
	}
...
Message *DataIdVar::p_getMember(const char *s, unsigned n)
{
	// caller guarantee: s[n] == '.'
	return 0;
}

Message *DataIdVar::p_getMember(const char *s, unsigned n, unsigned x)
{
	// caller guarantee: s[n] == '['
	return 0;
}

void XPlaneConfig::clear()
{
	m_ip4addr = 0;
	m_refport = 0;
	m_dataport = 0;
...
void XPlaneConfig::toASCII(std::ostream &o, bool full, size_t indent) const
{
	o << "{ // XPlaneConfig";
	++indent;
	if (full || has_ip4addr()) {
		ascii_indent(o,indent,"ip4addr");
		ip4_to_ascii(o,m_ip4addr);
		o << ';';
	}
	if (full || has_refport()) {
		ascii_numeric(o, indent, "refport", m_refport);
	}
	if (full || has_dataport()) {
		ascii_numeric(o, indent, "dataport", m_dataport);
	}
	if (full || (0 != m_dataids.size())) {
		gen_indent(o,indent);
		size_t s_dataids = m_dataids.size();
		o << "dataids[" << s_dataids << "] = {";
		++indent;
		for (size_t i = 0, e = s_dataids; i != e; ++i) {
			gen_indent(o,indent);
			o << '[' << i << "]: ";
			m_dataids[i].toASCII(o,full,indent);
		}
		--indent;
		gen_indent(o,indent);
		o << '}';
	}
	if (full || (0 != m_drefs.size())) {
		gen_indent(o,indent);
		size_t s_drefs = m_drefs.size();
		o << "drefs[" << s_drefs << "] = {";
		++indent;
		for (size_t i = 0, e = s_drefs; i != e; ++i) {
			gen_indent(o,indent);
			o << '[' << i << "]: ";
			m_drefs[i].toASCII(o,full,indent);
		}
		--indent;
		gen_indent(o,indent);
		o << '}';
...
void XPlaneConfig::toPbt(std::ostream &o, bool full, size_t indent) const
{
	o << '{';
	++indent;
	if (full || has_ip4addr()) {
		pbt_numeric(o, indent, "ip4addr", m_ip4addr);
		o << "\t# ";
		ip4_to_ascii(o,m_ip4addr);
	}
	if (full || has_refport()) {
		pbt_numeric(o, indent, "refport", m_refport);
	}
	if (full || has_dataport()) {
		pbt_numeric(o, indent, "dataport", m_dataport);
	}
	if (full || (0 != m_dataids.size())) {
		gen_indent(o,indent);
		size_t s_dataids = m_dataids.size();
		...
		for (size_t i = 0, e = s_dataids; i != e; ++i) {
			gen_indent(o,indent);
			m_dataids[i].toPbt(o,full,indent);
		}
		--indent;
		gen_indent(o,indent);
		o << ']';
...
ssize_t XPlaneConfig::fromMemory(const void *b, ssize_t s)
{
	const uint8_t *a = (const uint8_t *)b;
	const uint8_t *e = a + s;
	while (a < e) {
		...
		a += fn;
		switch (fid) {
		case 0xd:	// ip4addr id 1, type uint32_t, coding 32bit
			if ((a+3) >= e)
				return -336;
			set_ip4addr((uint32_t) read_u32(a));
			a += 4;
			break;
		case 0x14:	// refport id 2, type uint16_t, coding 16bit
			if ((a+1) >= e)
				return -337;
			set_refport((uint16_t) read_u16(a));
			a += 2;
			break;
		case 0x1c:	// dataport id 3, type uint16_t, coding 16bit
			if ((a+1) >= e)
				return -338;
			set_dataport((uint16_t) read_u16(a));
			a += 2;
			break;
		case 0x22:	// dataids id 4, type DataIdVar, coding byte[]
			{
				varint_t v;
				int n = read_varint(a,e-a,&v);
				a += n;
				if ((n <= 0) || ((a+v) > e))
					return -339;
				m_dataids.emplace_back();
				if (v != 0) {
					n = m_dataids.back().fromMemory((const uint8_t*)a,v);
					if (n < 0)
						return n;
					if (n != (ssize_t)v)
						return -340;
			...
			break;
		case 0x2a:	// drefs id 5, type Dref, coding byte[]
			{
				varint_t v;
				int n = read_varint(a,e-a,&v);
				a += n;
				if ((n <= 0) || ((a+v) > e))
					return -341;
				m_drefs.emplace_back();
				if (v != 0) {
					n = m_drefs.back().fromMemory((const uint8_t*)a,v);
					if (n < 0)
						return n;
					if (n != (ssize_t)v)
						return -342;
...
ssize_t XPlaneConfig::toMemory(uint8_t *b, ssize_t s) const
{
	assert(s >= 0);
	uint8_t *a = b, *e = b + s;
	signed n;
	// has ip4addr?
	if (0 != (p_validbits & ((uint8_t)1U << vb_ip4addr))) {
		// 'ip4addr': id=1, encoding=32bit, tag=0xd
		if (5 > (e-a))
			return -345;
		*a++ = 0xd;
		if ((e-a) < 4)
			return -346;
		write_u32(a,(uint32_t)m_ip4addr);
		a += 4;
	}
	// has refport?
	if (m_refport != 0) {
		// 'refport': id=2, encoding=16bit, tag=0x14
		if (3 > (e-a))
			return -347;
		*a++ = 0x14;
		write_u16(a,m_refport);
		a += 2;
	}
	// has dataport?
	if (m_dataport != 0) {
		// 'dataport': id=3, encoding=16bit, tag=0x1c
		if (3 > (e-a))
			return -348;
		*a++ = 0x1c;
		write_u16(a,m_dataport);
		a += 2;
	}
	for (const auto &x : m_dataids) {
		// 'dataids': id=4, encoding=lenpfx, tag=0x22
		if (a >= e)
			return -349;
		*a++ = 0x22;
		ssize_t dataids_ws = x.calcSize();
		n = write_varint(a,e-a,dataids_ws);
		a += n;
		if ((n <= 0) || (dataids_ws > (e-a)))
			return -350;
		n = x.toMemory(a,e-a);
		a += n;
		assert(n == dataids_ws);
	}
	for (const auto &x : m_drefs) {
		// 'drefs': id=5, encoding=lenpfx, tag=0x2a
		if (a >= e)
			return -351;
		*a++ = 0x2a;
		ssize_t drefs_ws = x.calcSize();
		n = write_varint(a,e-a,drefs_ws);
		a += n;
		if ((n <= 0) || (drefs_ws > (e-a)))
			return -352;
		n = x.toMemory(a,e-a);
...
void XPlaneConfig::toWire(void (*put)(uint8_t)) const
{
	if (0 != (p_validbits & ((uint8_t)1U << vb_ip4addr))) {
		// 'ip4addr': id=1, encoding=32bit, tag=0xd
		put(0xd);	// 'ip4addr': id=1
		send_u32(put,(uint32_t)m_ip4addr);
	}
	if (m_refport != 0) {
		// 'refport': id=2, encoding=16bit, tag=0x14
		put(0x14);	// 'refport': id=2
		send_u16(put,m_refport);
	}
	if (m_dataport != 0) {
		// 'dataport': id=3, encoding=16bit, tag=0x1c
		put(0x1c);	// 'dataport': id=3
		send_u16(put,m_dataport);
	}
	for (size_t x = 0, y = m_dataids.size(); x != y; ++x) {
		// 'dataids': id=4, encoding=lenpfx, tag=0x22
		put(0x22);	// 'dataids': id=4
		send_varint(put,m_dataids[x].calcSize());
		m_dataids[x].toWire(put);
	}
	for (size_t x = 0, y = m_drefs.size(); x != y; ++x) {
		// 'drefs': id=5, encoding=lenpfx, tag=0x2a
		put(0x2a);	// 'drefs': id=5
		send_varint(put,m_drefs[x].calcSize());
		m_drefs[x].toWire(put);
	}
}

void XPlaneConfig::toString(std::string &put) const
{
	if (0 != (p_validbits & ((uint8_t)1U << vb_ip4addr))) {
		// 'ip4addr': id=1, encoding=32bit, tag=0xd
		put.push_back(0xd);	// 'ip4addr': id=1
		send_u32(put,(uint32_t)m_ip4addr);
	}
	if (m_refport != 0) {
		// 'refport': id=2, encoding=16bit, tag=0x14
		put.push_back(0x14);	// 'refport': id=2
		send_u16(put,m_refport);
	}
	if (m_dataport != 0) {
		// 'dataport': id=3, encoding=16bit, tag=0x1c
		put.push_back(0x1c);	// 'dataport': id=3
		send_u16(put,m_dataport);
	}
	for (size_t x = 0, y = m_dataids.size(); x != y; ++x) {
		// 'dataids': id=4, encoding=lenpfx, tag=0x22
		put.push_back(0x22);	// 'dataids': id=4
		send_varint(put,m_dataids[x].calcSize());
		m_dataids[x].toString(put);
	}
	for (size_t x = 0, y = m_drefs.size(); x != y; ++x) {
		// 'drefs': id=5, encoding=lenpfx, tag=0x2a
		put.push_back(0x2a);	// 'drefs': id=5
		send_varint(put,m_drefs[x].calcSize());
		m_drefs[x].toString(put);
	}
}

void XPlaneConfig::toJSON(std::ostream &json, bool full, unsigned indLvl) const
{
	char fsep = '{';
	++indLvl;
	if (full || has_ip4addr()) {
		fsep = json_indent(json,indLvl,fsep,"ip4addr");
		json.put('"');
		ip4_to_ascii(json,m_ip4addr);
		json.put('"');
	}
	if (full || has_refport()) {
		fsep = json_indent(json,indLvl,fsep,"refport");
		json << m_refport;
	}
	if (full || has_dataport()) {
		fsep = json_indent(json,indLvl,fsep,"dataport");
		json << m_dataport;
	}
	if (size_t s = m_dataids.size()) {
		fsep = json_indent(json,indLvl,fsep);
		indLvl += 2;
		json << "\"dataids\":[\n";
		size_t i = 0;
		for (;;) {
			json_indent(json,indLvl,0);
			m_dataids[i].toJSON(json,full,indLvl);
			++i;
			if (i == s)
				break;
			json << ",\n";
	...
	if (size_t s = m_drefs.size()) {
		...
		for (;;) {
			json_indent(json,indLvl,0);
			m_drefs[i].toJSON(json,full,indLvl);
			++i;
			if (i == s)
				break;
			json << ",\n";
...
size_t XPlaneConfig::calcSize() const
{
	size_t r = 0;	// required size, default is fixed length
	// optional fixed32 ip4addr, id 1
	if (0 != (p_validbits & ((uint8_t)1U << vb_ip4addr))) {
		r += 5;
	}
	// optional fixed16 refport, id 2
	if (m_refport != 0) {
		r += 3;
	}
	// optional fixed16 dataport, id 3
	if (m_dataport != 0) {
		r += 3;
	}
	// repeated DataIdVar dataids, id 4
	// repeated message dataids
	for (size_t x = 0, y = m_dataids.size(); x < y; ++x) {
		size_t s = m_dataids[x].calcSize();
		r += wiresize(s);
		r += s + 1 /* tag(dataids) 0x20 */;
	}
	// repeated Dref drefs, id 5
	// repeated message drefs
	for (size_t x = 0, y = m_drefs.size(); x < y; ++x) {
		size_t s = m_drefs[x].calcSize();
		r += wiresize(s);
		r += s + 1 /* tag(drefs) 0x28 */;
	}
	return r;
}

bool XPlaneConfig::operator != (const XPlaneConfig &r) const
{
	if (p_validbits != r.p_validbits)
		return true;
	if (has_ip4addr() && (m_ip4addr != r.m_ip4addr))
		return true;
	if (has_refport() && (m_refport != r.m_refport))
		return true;
	if (has_dataport() && (m_dataport != r.m_dataport))
		return true;
	if (m_dataids != r.m_dataids)
		return true;
	if (m_drefs != r.m_drefs)
		return true;
	return false;
}
 ...
 *         an error.
 */
int XPlaneConfig::setByName(const char *name, const char *value)
{
	if (0 == strcmp(name,"ip4addr")) {
		if (value == 0) {
			clear_ip4addr();
			return 0;
		}
		int r = parse_ipv4(&m_ip4addr,value);
		if (r > 0)
			p_validbits |= ((uint8_t)1U << 0);
		return r;
	}
	if (0 == strcmp(name,"refport")) {
		if (value == 0) {
			clear_refport();
			return 0;
		}
		int r = parse_ascii_u16(&m_refport,value);
		return r;
	}
	if (0 == strcmp(name,"dataport")) {
		if (value == 0) {
			clear_dataport();
			return 0;
		}
		int r = parse_ascii_u16(&m_dataport,value);
		return r;
	}
	if (0 == memcmp(name,"dataids",7)) {
		if ((name[7] == 0) && (value == 0)) {
			clear_dataids();
			return 0;
		} else if (name[7] == '[') {
			...
			if ((name[8] == '+') && (name[9] == ']')) {
				...
				if (m_dataids.size() <= x)
					return -354;
				if ((idxe[1] == 0) && (value == 0)) {
					m_dataids.erase(m_dataids.begin()+x);
					return 0;
				}
			}
			if (idxe[1] != '.')
				return -355;
			return m_dataids[x].setByName(idxe+2,value);
		}
	}
	if (0 == memcmp(name,"drefs",5)) {
		if ((name[5] == 0) && (value == 0)) {
			clear_drefs();
			return 0;
		} else if (name[5] == '[') {
			...
			if ((name[6] == '+') && (name[7] == ']')) {
				...
				if (m_drefs.size() <= x)
					return -357;
				if ((idxe[1] == 0) && (value == 0)) {
					m_drefs.erase(m_drefs.begin()+x);
					return 0;
				}
			}
			if (idxe[1] != '.')
				return -358;
			return m_drefs[x].setByName(idxe+2,value);
		}
	}
	return -359;
}

Message *XPlaneConfig::p_getMember(const char *s, unsigned n)
{
	// caller guarantee: s[n] == '.'
	return 0;
}

Message *XPlaneConfig::p_getMember(const char *s, unsigned n, unsigned x)
{
	// caller guarantee: s[n] == '['
	if ((n == 7) && (0 == memcmp("dataids",s,7))) {
		if (x < m_dataids.size())
			return &m_dataids[x];
	} else if ((n == 5) && (0 == memcmp("drefs",s,5))) {
		if (x < m_drefs.size())
			return &m_drefs[x];
	}
	return 0;
...
void NodeConfig::toASCII(std::ostream &o, bool full, size_t indent) const
{
	o << "{ // NodeConfig";
	++indent;
	if (full || has_magic()) {
	...
	if (full || has_station()) {
		...
		m_station.toASCII(o,full,indent);
	}
	if (full || has_softap()) {
		ascii_indent(o,indent,"softap");
		m_softap.toASCII(o,full,indent);
	}
	if (full || (0 != m_dns_server.size())) {
		gen_indent(o,indent);
	...
	if (full || has_influx()) {
		...
		m_influx.toASCII(o,full,indent);
	}
	...
	if (full || (0 != m_holidays.size())) {
		...
		for (size_t i = 0, e = s_holidays; i != e; ++i) {
			gen_indent(o,indent);
			o << '[' << i << "]: ";
			m_holidays[i].toASCII(o,full,indent);
		}
		--indent;
		gen_indent(o,indent);
		o << '}';
	}
	if (full || (0 != m_at_actions.size())) {
		gen_indent(o,indent);
		...
		for (size_t i = 0, e = s_at_actions; i != e; ++i) {
			gen_indent(o,indent);
			o << '[' << i << "]: ";
			m_at_actions[i].toASCII(o,full,indent);
		}
		--indent;
		gen_indent(o,indent);
		o << '}';
	...
	if (full || (0 != m_triggers.size())) {
		...
		for (size_t i = 0, e = s_triggers; i != e; ++i) {
			gen_indent(o,indent);
			o << '[' << i << "]: ";
			m_triggers[i].toASCII(o,full,indent);
		}
		--indent;
		gen_indent(o,indent);
		o << '}';
	...
	if (full || (0 != m_terminal.size())) {
		...
		for (size_t i = 0, e = s_terminal; i != e; ++i) {
			gen_indent(o,indent);
			o << '[' << i << "]: ";
			m_terminal[i].toASCII(o,full,indent);
		}
		--indent;
		gen_indent(o,indent);
		o << '}';
	...
	if (full || has_ftpd()) {
		...
		m_ftpd.toASCII(o,full,indent);
	}
	if (full || has_httpd()) {
		ascii_indent(o,indent,"httpd");
		m_httpd.toASCII(o,full,indent);
	}
	if (full || has_otasrv()) {
		ascii_string(o,indent,m_otasrv.data(),m_otasrv.size(),"otasrv");
	}
	if (full || has_screen()) {
		ascii_indent(o,indent,"screen");
		m_screen.toASCII(o,full,indent);
	}
	if (full || has_history()) {
		ascii_numeric(o, indent, "history", m_history);
	}
	if (full || (0 != m_timefuses.size())) {
		gen_indent(o,indent);
		...
		for (size_t i = 0, e = s_timefuses; i != e; ++i) {
			gen_indent(o,indent);
			o << '[' << i << "]: ";
			m_timefuses[i].toASCII(o,full,indent);
		}
		--indent;
		gen_indent(o,indent);
		o << '}';
	}
	if (full || (0 != m_statemachs.size())) {
		gen_indent(o,indent);
		...
		for (size_t i = 0, e = s_statemachs; i != e; ++i) {
			gen_indent(o,indent);
			o << '[' << i << "]: ";
			m_statemachs[i].toASCII(o,full,indent);
		}
		--indent;
		gen_indent(o,indent);
		o << '}';
	...
	if (full || (0 != m_app_params.size())) {
		...
		for (size_t i = 0, e = s_app_params; i != e; ++i) {
			gen_indent(o,indent);
			o << '[' << i << "]: ";
			m_app_params[i].toASCII(o,full,indent);
		}
		--indent;
		gen_indent(o,indent);
		o << '}';
	}
	if (full || (0 != m_thresholds.size())) {
		gen_indent(o,indent);
		...
		for (size_t i = 0, e = s_thresholds; i != e; ++i) {
			gen_indent(o,indent);
			o << '[' << i << "]: ";
			m_thresholds[i].toASCII(o,full,indent);
		}
		--indent;
		gen_indent(o,indent);
		o << '}';
	}
	if (full || (0 != m_luafiles.size())) {
		gen_indent(o,indent);
	...
	if (full || has_lua_disable()) {
		ascii_bool(o, indent, "lua_disable", m_lua_disable);
	}
	if (full || (0 != m_owdevices.size())) {
		gen_indent(o,indent);
		...
		for (size_t i = 0, e = s_owdevices; i != e; ++i) {
			gen_indent(o,indent);
			o << '[' << i << "]: ";
			m_owdevices[i].toASCII(o,full,indent);
		}
		--indent;
		gen_indent(o,indent);
		o << '}';
	...
	if (full || has_xplane()) {
		ascii_indent(o,indent,"xplane");
		m_xplane.toASCII(o,full,indent);
	}
	--indent;
	gen_indent(o,indent);
	o << '}';
...
void NodeConfig::toPbt(std::ostream &o, bool full, size_t indent) const
{
	o << '{';
	++indent;
	if (full || has_magic()) {
	...
	if (full || has_nodename()) {
		pbt_string(o,indent,m_nodename.data(),m_nodename.size(),"nodename");
	}
	if (full || has_pass_hash()) {
		ascii_bytes(o,(const uint8_t*)m_pass_hash.data(),m_pass_hash.size(),indent,"pass_hash");
	}
	if (full || has_cpu_freq()) {
		pbt_numeric(o, indent, "cpu_freq", m_cpu_freq);
	}
	if (full || has_station()) {
		pbt_indent(o,indent,"station");
		m_station.toPbt(o,full,indent);
	}
	if (full || has_softap()) {
		pbt_indent(o,indent,"softap");
		m_softap.toPbt(o,full,indent);
	}
	if (full || (0 != m_dns_server.size())) {
		gen_indent(o,indent);
		size_t s_dns_server = m_dns_server.size();
		...
		for (size_t i = 0, e = s_dns_server; i != e; ++i) {
			gen_indent(o,indent);
			pbt_string(o,indent,m_dns_server[i].data(),m_dns_server[i].size(),0);
		}
		--indent;
		gen_indent(o,indent);
		o << ']';
	...
	if (full || has_syslog_host()) {
		pbt_string(o,indent,m_syslog_host.data(),m_syslog_host.size(),"syslog_host");
	}
	if (full || has_sntp_server()) {
		pbt_string(o,indent,m_sntp_server.data(),m_sntp_server.size(),"sntp_server");
	}
	if (full || has_timezone()) {
		pbt_string(o,indent,m_timezone.data(),m_timezone.size(),"timezone");
	}
	if (full || has_mqtt()) {
		pbt_indent(o,indent,"mqtt");
		m_mqtt.toPbt(o,full,indent);
	}
	if (full || has_dmesg_size()) {
		pbt_numeric(o, indent, "dmesg_size", m_dmesg_size);
	}
	if (full || has_influx()) {
		pbt_indent(o,indent,"influx");
		m_influx.toPbt(o,full,indent);
	}
	if (full || has_station2ap_time()) {
		pbt_numeric(o, indent, "station2ap_time", m_station2ap_time);
	}
	if (full || has_domainname()) {
		pbt_string(o,indent,m_domainname.data(),m_domainname.size(),"domainname");
	}
	if (full || (0 != m_holidays.size())) {
		gen_indent(o,indent);
		size_t s_holidays = m_holidays.size();
		...
		for (size_t i = 0, e = s_holidays; i != e; ++i) {
			gen_indent(o,indent);
			m_holidays[i].toPbt(o,full,indent);
		}
		--indent;
		gen_indent(o,indent);
		o << ']';
	...
	if (full || (0 != m_at_actions.size())) {
		...
		for (size_t i = 0, e = s_at_actions; i != e; ++i) {
			gen_indent(o,indent);
			m_at_actions[i].toPbt(o,full,indent);
		}
		--indent;
		gen_indent(o,indent);
		o << ']';
	...
	if (full || has_actions_enable()) {
		pbt_numeric(o, indent, "actions_enable", m_actions_enable);
	}
	if (full || (0 != m_triggers.size())) {
		gen_indent(o,indent);
		size_t s_triggers = m_triggers.size();
		...
		for (size_t i = 0, e = s_triggers; i != e; ++i) {
			gen_indent(o,indent);
			m_triggers[i].toPbt(o,full,indent);
		}
		--indent;
		gen_indent(o,indent);
		o << ']';
	...
	if (full || (0 != m_terminal.size())) {
		...
		for (size_t i = 0, e = s_terminal; i != e; ++i) {
			gen_indent(o,indent);
			m_terminal[i].toPbt(o,full,indent);
		}
		--indent;
		gen_indent(o,indent);
		o << ']';
	...
	if (full || has_udp_ctrl_port()) {
		pbt_numeric(o, indent, "udp_ctrl_port", m_udp_ctrl_port);
	}
	if (full || (0 != m_debugs.size())) {
		gen_indent(o,indent);
		size_t s_debugs = m_debugs.size();
		...
		for (size_t i = 0, e = s_debugs; i != e; ++i) {
			gen_indent(o,indent);
			pbt_string(o,indent,m_debugs[i].data(),m_debugs[i].size(),0);
		}
		--indent;
		gen_indent(o,indent);
		o << ']';
	...
	if (full || has_ftpd()) {
		pbt_indent(o,indent,"ftpd");
		m_ftpd.toPbt(o,full,indent);
	}
	if (full || has_httpd()) {
		pbt_indent(o,indent,"httpd");
		m_httpd.toPbt(o,full,indent);
	}
	if (full || has_otasrv()) {
		pbt_string(o,indent,m_otasrv.data(),m_otasrv.size(),"otasrv");
	}
	if (full || has_screen()) {
		pbt_indent(o,indent,"screen");
		m_screen.toPbt(o,full,indent);
	}
	if (full || has_history()) {
		pbt_numeric(o, indent, "history", m_history);
	}
	if (full || (0 != m_timefuses.size())) {
		gen_indent(o,indent);
		size_t s_timefuses = m_timefuses.size();
		...
		for (size_t i = 0, e = s_timefuses; i != e; ++i) {
			gen_indent(o,indent);
			m_timefuses[i].toPbt(o,full,indent);
		}
		--indent;
		gen_indent(o,indent);
		o << ']';
	...
	if (full || (0 != m_statemachs.size())) {
		...
		for (size_t i = 0, e = s_statemachs; i != e; ++i) {
			gen_indent(o,indent);
			m_statemachs[i].toPbt(o,full,indent);
		}
		--indent;
		gen_indent(o,indent);
		o << ']';
	...
	if (full || has_dim_step()) {
		pbt_numeric(o, indent, "dim_step", m_dim_step);
	}
	if (full || has_pwm_freq()) {
		pbt_numeric(o, indent, "pwm_freq", m_pwm_freq);
	}
	if (full || (0 != m_app_params.size())) {
		gen_indent(o,indent);
		size_t s_app_params = m_app_params.size();
		...
		for (size_t i = 0, e = s_app_params; i != e; ++i) {
			gen_indent(o,indent);
			m_app_params[i].toPbt(o,full,indent);
		}
		--indent;
		gen_indent(o,indent);
		o << ']';
	...
	if (full || (0 != m_thresholds.size())) {
		...
		for (size_t i = 0, e = s_thresholds; i != e; ++i) {
			gen_indent(o,indent);
			m_thresholds[i].toPbt(o,full,indent);
		}
		--indent;
		gen_indent(o,indent);
		o << ']';
	...
	if (full || (0 != m_luafiles.size())) {
		...
		for (size_t i = 0, e = s_luafiles; i != e; ++i) {
			gen_indent(o,indent);
			pbt_string(o,indent,m_luafiles[i].data(),m_luafiles[i].size(),0);
		}
		--indent;
		gen_indent(o,indent);
		o << ']';
	...
	if (full || has_lua_disable()) {
		pbt_bool(o, indent, "lua_disable", m_lua_disable);
	}
	if (full || (0 != m_owdevices.size())) {
		gen_indent(o,indent);
		size_t s_owdevices = m_owdevices.size();
		...
		for (size_t i = 0, e = s_owdevices; i != e; ++i) {
			gen_indent(o,indent);
			m_owdevices[i].toPbt(o,full,indent);
		}
		--indent;
		gen_indent(o,indent);
		o << ']';
	...
	if (full || has_xplane()) {
		pbt_indent(o,indent,"xplane");
		m_xplane.toPbt(o,full,indent);
	}
	--indent;
	gen_indent(o,indent);
	o << '}';
...
ssize_t NodeConfig::fromMemory(const void *b, ssize_t s)
{
	...
	while (a < e) {
		...
		case 0xca:	// httpd id 25, type FtpHttpConfig, coding byte[]
			...
			break;
		case 0xd2:	// otasrv id 26, type std::string, coding byte[]
			{
				varint_t v;
				int n = read_varint(a,e-a,&v);
				a += n;
			...
			break;
		case 0xda:	// screen id 27, type ScreenConfig, coding byte[]
			{
				varint_t v;
				int n = read_varint(a,e-a,&v);
				a += n;
				if ((n <= 0) || ((a+v) > e))
					return -398;
				if (v != 0) {
					n = m_screen.fromMemory((const uint8_t*)a,v);
					if (n < 0)
						return n;
					if (n != (ssize_t)v)
						return -399;
			...
			break;
		case 0xe0:	// history id 28, type uint64_t, coding varint
			{
				varint_t v;
				int n = read_varint(a,e-a,&v);
				if (n <= 0)
		...
		case 0x1e2:	// xplane id 60, type XPlaneConfig, coding byte[]
			{
				...
				if ((n <= 0) || ((a+v) > e))
					return -415;
				if (v != 0) {
					n = m_xplane.fromMemory((const uint8_t*)a,v);
					if (n < 0)
						return n;
					if (n != (ssize_t)v)
						return -416;
...
ssize_t NodeConfig::toMemory(uint8_t *b, ssize_t s) const
{
	...
	// has screen?
	if (0 != (p_validbits & ((uint16_t)1U << vb_screen))) {
		// 'screen': id=27, encoding=lenpfx, tag=0xda
		if (2 > (e-a))
			return -469;
		*a++ = 0xda;
		*a++ = 0x1;
		ssize_t screen_ws = m_screen.calcSize();
		n = write_varint(a,e-a,screen_ws);
		a += n;
		if ((n <= 0) || (screen_ws > (e-a)))
			return -470;
		n = m_screen.toMemory(a,e-a);
		a += n;
		assert(n == screen_ws);
	}
	// has history?
	if (0 != (p_validbits & ((uint16_t)1U << vb_history))) {
		// 'history': id=28, encoding=varint, tag=0xe0
		if (2 > (e-a))
			return -471;
		*a++ = 0xe0;
		*a++ = 0x1;
		n = write_varint(a,e-a,m_history);
		if (n <= 0)
			return -472;
		a += n;
	}
...
void NodeConfig::toWire(void (*put)(uint8_t)) const
{
	...
	if (!m_otasrv.empty()) {
		...
		send_varint(put,otasrv_s);
		send_bytes(put,(const uint8_t*) m_otasrv.data(),otasrv_s);
	}
	if (0 != (p_validbits & ((uint16_t)1U << vb_screen))) {
		// 'screen': id=27, encoding=lenpfx, tag=0xda
		put(0xda);
		put(0x1);
		send_varint(put,m_screen.calcSize());
		m_screen.toWire(put);
	}
	if (0 != (p_validbits & ((uint16_t)1U << vb_history))) {
		// 'history': id=28, encoding=varint, tag=0xe0
	...
	// 'signals' is deprecated. Therefore no data will be written.
	// 'functions' is deprecated. Therefore no data will be written.
	...
	// 'lightctrl' is deprecated. Therefore no data will be written.
	if (m_pwm_freq != 0) {
		// 'pwm_freq': id=39, encoding=varint, tag=0x138
		put(0xb8);
		put(0x2);
	...
	if (0 != (p_validbits & ((uint16_t)1U << vb_xplane))) {
		...
		put(0xe2);
		put(0x3);
		send_varint(put,m_xplane.calcSize());
		m_xplane.toWire(put);
	}
}
...
void NodeConfig::toString(std::string &put) const
{
	...
	if (!m_otasrv.empty()) {
		...
		size_t otasrv_s = m_otasrv.size();
		send_varint(put,otasrv_s);
		put.append((const char *)(const uint8_t*) m_otasrv.data(),otasrv_s);
	}
	if (0 != (p_validbits & ((uint16_t)1U << vb_screen))) {
		// 'screen': id=27, encoding=lenpfx, tag=0xda
		put.push_back(0xda);
	...
	if (0 != (p_validbits & ((uint16_t)1U << vb_xplane))) {
		...
		put.push_back(0x3);
		send_varint(put,m_xplane.calcSize());
		m_xplane.toString(put);
	}
}

void NodeConfig::toJSON(std::ostream &json, bool full, unsigned indLvl) const
{
	char fsep = '{';
	++indLvl;
	if (full || has_magic()) {
	...
	if (full || has_screen()) {
		fsep = json_indent(json,indLvl,fsep,"screen");
		m_screen.toJSON(json,full,indLvl);
	}
	if (full || has_history()) {
		fsep = json_indent(json,indLvl,fsep,"history");
		json << m_history;
	}
	...
	if (size_t s = m_statemachs.size()) {
		...
		for (;;) {
			json_indent(json,indLvl,0);
			m_statemachs[i].toJSON(json,full,indLvl);
			++i;
			if (i == s)
				break;
			json << ",\n";
...
size_t NodeConfig::calcSize() const
{
	...
	if (0 != (p_validbits & ((uint16_t)1U << vb_screen))) {
		size_t screen_s = m_screen.calcSize();
		r += screen_s + wiresize(screen_s) + 2 /* tag(screen) 0xd8 */;
	}
	// optional unsigned history, id 28
	if (0 != (p_validbits & ((uint16_t)1U << vb_history))) {
		r += wiresize((varint_t)m_history) + 2 /* tag(history) 0xe0 */;
	}
...
bool NodeConfig::operator != (const NodeConfig &r) const
{
	...
	if (has_otasrv() && (m_otasrv != r.m_otasrv))
		return true;
	if (has_screen() && (m_screen != r.m_screen))
		return true;
	if (has_history() && (m_history != r.m_history))
		return true;
...
int NodeConfig::setByName(const char *name, const char *value)
{
	...
	if (0 == memcmp(name,"screen",6)) {
		if ((name[6] == 0) && (value == 0)) {
			clear_screen();
			return 0;
		} else if (name[6] == '.') {
			p_validbits |= ((uint16_t)1U << 11);
			return m_screen.setByName(name+7,value);
		}
	}
	if (0 == strcmp(name,"history")) {
		...
		int r = parse_ascii_u64(&m_history,value);
		if (r > 0)
...
Message *NodeConfig::p_getMember(const char *s, unsigned n)
{
	// caller guarantee: s[n] == '.'
	if ((n == 7) && (0 == memcmp("station",s,7)))
		return &m_station;
	if ((n == 6) && (0 == memcmp("softap",s,6)))
		return &m_softap;
	if ((n == 4) && (0 == memcmp("mqtt",s,4)))
		return &m_mqtt;
	if ((n == 6) && (0 == memcmp("influx",s,6)))
		return &m_influx;
	if ((n == 4) && (0 == memcmp("ftpd",s,4)))
		return &m_ftpd;
	if ((n == 5) && (0 == memcmp("httpd",s,5)))
		return &m_httpd;
	if ((n == 6) && (0 == memcmp("screen",s,6)))
		return &m_screen;
	if ((n == 6) && (0 == memcmp("xplane",s,6)))
		return &m_xplane;
	return 0;
}

Message *NodeConfig::p_getMember(const char *s, unsigned n, unsigned x)
{
	// caller guarantee: s[n] == '['
	if ((n == 8) && (0 == memcmp("holidays",s,8))) {
		if (x < m_holidays.size())
			return &m_holidays[x];
	} else if ((n == 10) && (0 == memcmp("at_actions",s,10))) {
		if (x < m_at_actions.size())
			return &m_at_actions[x];
	} else if ((n == 8) && (0 == memcmp("triggers",s,8))) {
		if (x < m_triggers.size())
			return &m_triggers[x];
	} else if ((n == 4) && (0 == memcmp("uart",s,4))) {
		if (x < m_uart.size())
			return &m_uart[x];
	} else if ((n == 8) && (0 == memcmp("terminal",s,8))) {
		if (x < m_terminal.size())
			return &m_terminal[x];
	} else if ((n == 9) && (0 == memcmp("timefuses",s,9))) {
		if (x < m_timefuses.size())
			return &m_timefuses[x];
	} else if ((n == 10) && (0 == memcmp("statemachs",s,10))) {
		if (x < m_statemachs.size())
			return &m_statemachs[x];
	} else if ((n == 10) && (0 == memcmp("app_params",s,10))) {
		if (x < m_app_params.size())
			return &m_app_params[x];
	} else if ((n == 10) && (0 == memcmp("thresholds",s,10))) {
		if (x < m_thresholds.size())
			return &m_thresholds[x];
	} else if ((n == 9) && (0 == memcmp("owdevices",s,9))) {
