...
Message *LuaConfig::p_getMember(const char *s, unsigned n)
{
	// caller guarantee: s[n] == '.'
	return 0;
}

Message *LuaConfig::p_getMember(const char *s, unsigned n, unsigned x)
{
	// caller guarantee: s[n] == '['
	return 0;
}
...
Message *NodeConfig::p_getMember(const char *s, unsigned n, unsigned x)
{
	...
	if ((n == 8) && (0 == memcmp("holidays",s,8))) {
		...
		#ifdef CONFIG_SIGNAL_PROC
	} else if ((n == 7) && (0 == memcmp("signals",s,7))) {
		if (x < m_signals.size())
			return &m_signals[x];
		#endif // CONFIG_SIGNAL_PROC
		#ifdef CONFIG_SIGNAL_PROC
	} else if ((n == 9) && (0 == memcmp("functions",s,9))) {
