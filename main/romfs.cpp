...
void *romfs_mmap(int i)
{
	if ((i < 0) || (i >= NumEntries))
		return 0;
	return RomfsBaseAddr+Entries[i].offset;
}
#endif
...
void romfs_setup()
{
	...
	while (pi != 0) {
		...
		if (0 == strcmp(magic,ROMFS_MAGIC))
			break;
		pi = esp_partition_next(pi);
	}
	if (p == 0) {
		log_dbug(TAG,"no partition");
		return;
	}
	log_dbug(TAG,"partition %s",p->label);

	RomfsBaseAddr = p->address;
	RomfsSpace = p->size;
#ifdef CONFIG_ENABLE_FLASH_MMAP
	spi_flash_mmap_handle_t handle;
	if (esp_err_t e = spi_flash_mmap(p->address,p->size,SPI_FLASH_MMAP_DATA,&RomfsBaseAddr,&handle)) {
